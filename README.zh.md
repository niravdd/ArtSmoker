...
2. **选择模型和设置** —— 从 Amazon Bedrock 上所有可用的文生图模型中选择，设定尺寸、质量等级和区域。或使用 "All Available Models" 在所有已启用的模型上同时生成，进行并排比较。
...
- 📝 **Prompt Templates** —— 14 个可编辑的 LLM 指令提示词、AI 辅助优化、变量验证和自动修复
...
- 🔧 **Self-Hosted Models** —— 在您自己的 AWS 账户中将开源模型（FLUX.1、Real-ESRGAN、RMBG、SAM 2 等）部署到 Amazon SageMaker。一键部署、异步缩容至零或常驻运行，与所有工作室完全集成

### 📝 1.2 屏幕截图

**2D Image Studio** —— 左侧为设置，右侧为带 AI 增强的提示词，下方为模型比较结果。All Available Models 模式在所有已启用的图像模型上同时生成。
...
从模型下拉菜单中选择 **"All Available Models"**，可在所有已启用的图像模型上同时生成提示词 —— 每个模型一张图像。这可以直接并排比较 Nova Canvas、Titan Image、SD 3.5 Large 和 Stable Image Ultra 对同一提示词的不同诠释。每个模型独立运行：如果较严格的模型阻止了提示词，您仍然可以获得接受该提示词的模型的结果，每个选项卡上都有清晰的状态标签（成功、被审核阻止或失败）。

可选的 **"Model-optimized prompts"** 开关会针对每个模型的优势调整提示词，而不是向所有模型发送相同的提示词 —— 当您想从每个模型获得最佳输出而非直接比较时很有用。
