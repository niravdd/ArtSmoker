...
3. **获取真正不同的选项** —— 系统生成最多 5 个明显不同的创意概念（变化服装、情绪、光照、构图 —— 而非仅仅改变相机角度），每个概念最多 5 个种子变体（共 25 张图像）。用户指定的细节被锁定；AI 推断的细节则被大胆变化。 可见的**种子**控件让批次可复现 —— 相同的种子和设置可重新生成相同的图像，点击任意结果即可锚定其种子，从而只调整一项设置并从喜欢的结果分支。
