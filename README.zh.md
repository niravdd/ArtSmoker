...
![ArtSmoker 演示 — 从文本提示词到生产级 2D 美术，再到带完整纹理、为游戏引擎量身定制的 3D 模型](docs/images/artsmoker-walkthrough.gif)

## 📌 0. 概述

**ArtSmoker 让一个创意在几分钟内变成专为游戏引擎量身定制的美术资产 —— 无需您管理任何流程。** 用自然语言描述一个角色、道具、场景或主视觉，即可获得生产就绪的 2D 美术、完全纹理化的 3D 模型和视频 —— 全部匹配您项目的视觉标识，全部保留在您自己的环境中。最新的 AI 图像、编辑、3D 和视频模型都置于一个简洁、艺术家优先、带有真正创意控制的界面之后：ArtSmoker 为您运行整条生产流程，让您的团队专注于把控外观，而非摆弄底层机器。
...
- **2D、编辑与 3D —— 端到端** —— 先生成，再用局部重绘、扩展绘制、重新着色、搜索替换和参考引导编辑就地精调；将任意 2D 资产转换为**完全纹理化、为游戏引擎量身定制的 3D 模型**，可直接放入 Unity、Unreal 或 Blender —— 无需手动建模、UV 展开或纹理绘制。另外还有电影级视频和用于构思的多模型聊天工作室。
...
3. **获取真正不同的选项** —— 系统生成最多 5 个明显不同的创意概念（变化服装、情绪、光照、构图 —— 而非仅仅改变相机角度），每个概念最多 5 个种子变体（共 25 张图像）。用户指定的细节被锁定；AI 推断的细节则被大胆变化。 可见的**种子**控件让批次可复现 —— 相同的种子在最终提示词和设置也相同时可重新生成相同的图像，点击任意结果即可锚定其种子，从而只调整一项设置并从喜欢的结果分支。
...
- 🗂️ **合集（成套生成）** — 将一条提示词**或一张参考图**变成一整套风格一致的**不同资产**（如「维京风格国际象棋套装」「20 个 JRPG 角色」）。在 Image Studio 打开 **Collection** 开关——从 **Text** 标签（由你的提示词驱动）或 **Image-Inspired** 标签（由参考图的*外观*驱动美术方向）——专用的 **Collection Designer** 会将你的构思分解为共享的**美术方向**加一份主题一致的不同单品**清单**，每件都带有可编辑的、与模型无关的提示词。编辑美术方向即可重构整套；对单件进行锁定、重生成、新增或删除；然后一次性生成全部（每件 选项×变体），并实时显示设计＋预估成本。**可选一个模型得到统一的一套，或选多个模型让每件都由各模型分别生成**（跨模型对比）。可选的 **hero-**／**reference-anchor** 一致性会将整套的风格锁定到所选／所提供的图像。合集在 Gallery 中显示为**一张卡片**，打开合集查看器后，每件都可下钻到常规的 Asset Viewer（选项／变体／版本，以及该件的**合集谱系**）；你可以**固定代表每件的版本**、**一次性把整套转 3D**、**导出为按引擎打包的资源**，或**将整个合集重新载入 Image Studio** 以复查和重新生成。你的合集也会**直接显示在 Image Studio 中**(同样的卡片,就地打开查看器);并且在**产生任何费用之前**会根据提示词检查资产类型(合集为角色或游戏资产)——绝不会为错误类型的整套作品付费
...
- 🧊 **图像到 3D 生成** —— 将任何 Game Asset 或 Character 图像一键转换为带纹理的 3D 网格（GLB）。多视图合成 + 纹理烘焙生成为游戏引擎量身定制的资产。交互式 3D 查看器支持轨道旋转/缩放/平移
...
*3D 管线的截图 —— 生成、源图审查、面向游戏引擎定制的导出与变体 —— 展示在下方第 1.9 节（3D 模型生成）中，与其对应的功能相伴呈现。*
...
- **重绘参考图（Remix the reference）** —— 经典的基于强度的图生图：参考图的*像素*直接送入 Bedrock 模型（Stable Diffusion 3.5 Large 或 Stable Image Ultra —— 由注册表能力标志判定），并通过**强度旋钮**控制。细微档几乎保留构图、配色与氛围；大胆档则将其视作松散的灵感。选项数大于 1 时变为**强度阶梯** —— 每个强度一张卡片，从细微到大胆并排展示。*保留布局，但不保留身份*（人脸和产品会漂移 —— 需要身份精确请用忠于参考图模式）。输出尺寸跟随参考图。**始终可用** —— 无需部署，也没有视觉分析调用。

三种模式都需要一条指令，以便您始终掌控参考图的*用途*。参考引导生成与 Style Library（它将许多图像分析为可复用的风格档案）是分开的 —— 用它进行一次性的、由图像驱动的生成。
...
从任意 2D 图像生成完全纹理化的 3D 网格 —— 直接在 Asset Viewer 中操作。选择一张 **Game Asset** 或 **Character** 图像，打开 **3D Model** 选项卡，点击 Generate。结果是为游戏引擎量身定制的 GLB，您可以对其进行轨道旋转、缩放和下载 —— 无需手动建模、UV 展开或纹理绘制。

**先看最终成果：** ArtSmoker 生成的角色，导出为面向游戏引擎定制的 FBX 并在原生 Blender 中打开 —— LOD 链（LOD0–LOD3）完整保留在大纲视图中，纹理已绑定，无需重新绑骨或手工修复。下文将展示如何从一条文本提示词一步步走到这里。
...
| **TRELLIS.2（完整）** | 单一模型**同时**生成几何与 PBR 纹理（SLAT） | MIT 模型 + 权重 | ⚠️ **非商业烘焙** —— 网格导出硬性导入（hard-import）`nvdiffrast`（NVIDIA 1-Way Commercial）；商业使用前请先审查 | 最简路径；原型与非商业用途 |

> **商业用途：** **TripoSG + 纹理后端** 这条路径是商业安全的选择 —— 它的烘焙使用 **Kaolin (Apache-2.0)**。**完整 TRELLIS.2** 管线的模型本身为 MIT，但其 `o_voxel` 网格烘焙硬性导入（hard-import）`nvdiffrast`（NVIDIA Source Code License，对普通用户为非商业），且不修补上游就无法替换 —— 因此除非你已厘清该依赖，否则请将完整管线的输出视为非商业。许可与每一项依赖都会在部署对话框中、以及再次在生成时展示。
...
**基础设施：** 两条流程均通过相同的一键 Custom Models 流程部署，部署时的选择器会显示每个选项的许可、依赖表、实例基线以及预估成本/时间。完整 TRELLIS.2 流程的合理实例基线为 **`ml.g5.2xlarge`**（约 $1.52/小时；一次实测运行在 A10G 的 24 GB 显存上录得峰值约 4.78 GB 显存并顺利完成 —— 之所以选它而非 L40S 的 `g6e.xlarge`，是因为后者对该工作负载严重超配、成本更高）。更大的 `g6e` 规格则作为余量升级选项继续提供。端点在空闲时缩容至零 —— 作业间成本为 $0。首次冷启动会构建一次 CUDA 扩展（随后缓存至 S3 以加速重启）。在部署受限模型之前，对话框会**预先检查它将拉取的每一个仓库的 HuggingFace 访问权限**，并为每个仓库显示 ✓/✗ 以及确切的后续步骤 —— 这样您就不会在冷启动数分钟后才发现缺少某项许可接受。
...
![3D 模型查看器 — 各变体工具与面向游戏引擎定制的 FBX/USD 导出选项](docs/images/3d-model-viewer-export.png)

每个生成的 3D 模型都可以**按目标游戏引擎的要求准备好**后导出，直接在 Asset Viewer 的 3D 选项卡中完成：

- **目标引擎** —— 可选 Generic（glTF，Y 轴向上）、Unreal Engine（Z 轴向上）、Unity、Godot、Maya 或 3ds Max。FBX 和 USD 导出会按该引擎的正确上轴/前轴方向进行调整，模型导入后即为正立姿态 —— 无需手动修正旋转。
- **可选的预处理，由您决定**（每项都是独立的下拉选项 —— 绝不强制）：
  - **纹理打包** —— 按引擎生成对应的纹理集：Unreal 的 **ORM**（AO/粗糙度/金属度）、Unity 的 **Metallic + Smoothness 存于 alpha 通道**、Unity 的 **HDRP Mask Map**。选中后，导出内容会变成一个 ZIP，包含模型和一个 `textures/` 文件夹。
  - **LOD** —— 一条经过减面的 **LOD0–LOD3 链**（100/50/20/5%），带有 Unreal 可自动导入的真正 FBX LOD 组；`_LOD0…_LOD3` 命名同时符合 Unity 的约定。
  - **碰撞体** —— 凸包或 **CoACD 凸分解**，按各引擎的命名约定命名（Unreal 自动导入使用 `UCX_*`，Godot 使用 `-convcolonly` 后缀）。
  - **光照贴图 UV2** —— 通过智能投影生成的第二套 UV 通道，用于烘焙光照。
- **两步式流程** —— 对于尚不存在的组合，按钮显示为 **Generate FBX/USD/GLB**；点击后在服务器端进行转换（状态行会持续告知进度 —— 大模型可能需要一两分钟）。构建完成后，按钮变为带 ✓ 的 **Download**，即点即得。每个不同的组合都会被缓存 —— 绝不会重复生成。
- **"可供下载"标签** —— 3D 选项卡会列出当前版本已生成的所有组合，任意一项一键即可再次下载。
- **原始 GLB 不可侵犯** —— "Download GLB (original)" 始终返回生成流程输出的原封不动、逐字节一致的文件。经过处理的导出（包括烘焙了 LOD/碰撞体的处理版 GLB）作为单独命名的文件与其并存。
- **零安装配置** —— 转换在服务器端通过托管的无头 Blender 运行：若已有安装则复用，否则在首次使用时自动下载一份便携版（版本与更新见 Model Settings → Maintenance 选项卡）。最终用户无需安装任何东西。

### 📝 1.9.2 对 AI 生成 3D 的合理期待 —— 一份坦诚指南

图像到 3D 仍是一项年轻的技术，值得了解当今最好的模型（包括 ArtSmoker 所运行的这些）真正能交付什么 —— 以及不能交付什么。输出是一个**类似扫描物体的高密度网格**：最多约 100 万个无结构三角面，附带烘焙好的 PBR 纹理。近距离观察时会注意到表面特有的凹凸不平质感，而细薄特征（发丝、绑带、织物流苏）正是 AI 几何最薄弱的地方。这里**没有干净的四边形拓扑，没有适合动画的边循环，也没有绑定骨架** —— 这是整个行业的技术现状，并非某一款工具独有的局限。

**这些资产在哪些场景大放异彩 —— 哪些场景仍需美术师介入：**

| 使用场景 | 可直接使用？ |
|----------|---------------|
| 道具、环境杂物、场景陈设 | ✅ 可以 —— 可原样使用 |
| 背景 / 中远距离角色、人群 | ✅ 可以 —— 在一定距离下表面噪点会消失；配合 LOD 链使用 |
| 原型、粗模、预演、提案演示 | ✅ 可以 —— 可以说是最强的使用场景 |
| 移动端 / 风格化游戏 | ✅ 通常可以 —— 减面后的 LOD 很有帮助 |
| 主角角色、特写镜头、需要动画的角色 | ⚠️ 只是起点 —— 需计划由美术师进行重拓扑、清理和绑定 |

ArtSmoker 在原始网格之上附加的价值在于，一切都**按您的引擎正确打包就绪** —— 目标引擎对应的上轴方向、LOD 链、碰撞代理、引擎专属的纹理打包 —— 剩下的工作只是创作，而非搬砖式的管线杂务。

**在 Blender（或其他 DCC 工具）中检查导出文件时，有两处看起来很怪 —— 但都是正确的：**

- **生成时带了 LOD？** 文件中包含**层叠在一起的 4 份模型副本**（LOD0–3）。同时显示时会闪烁（z-fighting）、显得杂乱 —— 在大纲视图中隐藏 LOD1–3，仅凭 LOD0 判断质量即可。游戏引擎每次只显示一个 LOD，因此这种情况在引擎内绝不会发生。
- **生成时带了碰撞体？** 一层白色、块状的 `UCX_*` 网格外壳**包裹着模型** —— 那是物理代理，不是您的资产。隐藏这些对象即可看到里面的带纹理模型。引擎会自动将它们作为不可见碰撞体导入。

<a id="-193-using-a-model-commercially--who-to-pay-and-how"></a>

### 📝 1.9.3 商业化使用模型 —— 该向谁付费、如何付费

如果您喜欢某个模型的输出并希望将其用于商业用途，具体路径取决于**其创作者的盈利方式**。ArtSmoker 会在部署时展示每个模型的许可证；本节则是"接下来该怎么做？"的配套指南。*（已于 2026 年 8 月对照供应商网站和 HuggingFace 许可证文件核实 —— 许可条款变化很快，请务必确认供应商的最新条款。仅供参考 —— 见[免责声明](#disclaimer)。）*

您会遇到以下四种模式：

1. **已经归您所有（Apache-2.0 / MIT）** —— 商业使用已包含在内，且免费。没有可购买的许可证产品；创作者转而通过其自有托管 API 盈利。您唯一的义务是遵守声明/署名要求。
2. **做大之前免费（社区许可证）** —— **在一定阈值以下**（营收或月活跃用户数）商业使用已包含在内。超过阈值后，许可证本身会指示您向供应商申请企业授权 —— 这是一场销售洽谈，而不是商店购买。
3. **购买许可证，权重不变** —— HuggingFace 上的权重仅限非商业使用，但创作者单独出售**自托管商业许可证**。一旦持有该许可证，*您已经部署的同一套权重*即可合法用于商业用途 —— 在 ArtSmoker 中技术上无任何变化。
4. **门禁即付费墙** —— HuggingFace 仓库本身是受限（gated）的；与供应商签订商业协议后，您的 HF 账户即可解锁访问。此后 ArtSmoker 现有的 HuggingFace token + 受限仓库预检查流程可原样使用。

| 创作者 / 模型 | 模式 | 商业自托管的下一步 |
|---|---|---|
| **Alibaba** —— Qwen-Image、Qwen-Image-Edit | 1 | 无需购买（Apache-2.0）。保留许可证声明即可。 |
| **Microsoft** —— TRELLIS.2 · **VAST** —— TripoSG | 1 | 无需购买（MIT）。注意：上游依赖（如 Meta DINOv3）有各自的访问门禁/条款。 |
| **Black Forest Labs** —— FLUX.2 [klein] 4B | 1 | 无需购买 —— Apache-2.0，可免费商业使用。 |
| **Stability AI** —— SD 3.5（自托管） | 2 | 年总营收**低于 100 万美元**时免费且含商业使用（接受 HF 门禁*即是*许可证）。超过后许可证**自动终止** —— 请到 stability.ai/enterprise 申请企业许可证。**任何级别都必须带有 "Powered by Stability AI" 署名。** |
| **Tencent** —— HunyuanImage 3.0 / Hunyuan3D | 2 | 阈值以下免费且含商业使用 —— **注意阈值按模型区分**：HunyuanImage 3.0 = 1 亿 MAU；**Hunyuan3D-2.1 = 仅 100 万 MAU**（超过请发邮件至 hunyuan3d@tencent.com）。**在欧盟、英国、韩国完全不授权**，无论规模大小。 |
| **Black Forest Labs** —— FLUX.1/.2 [dev]、Kontext | 3 | 购买 **FLUX Commercial Weights License**（在 dashboard.bfl.ai/licensing 自助购买；各级别为按图像生成量封顶的订阅）。您继续使用同一套 HF 权重。注意相关义务：使用量报告、输出过滤、**不得将模型以 API 形式对外提供/转售**；除 Enterprise 级别外，新模型版本**不会**自动纳入授权范围。 |
| **Bria** —— FIBO、RMBG-2.0 | 4 | HF 门禁会立即授予**非商业**访问权限；商业自托管需要与 Bria 签订**付费协议**（购买表单链接见各模型卡片 / bria.ai）。不存在免费商业使用的阈值。获批后，照常通过 ArtSmoker 部署即可。 |

**这与 ArtSmoker 如何配合：**采购几乎不会改变任何技术层面的东西。对于模式 1–3，您部署的权重在购买前后完全相同 —— 变化的只是您持有的合约（请保管好您的许可证记录；ArtSmoker 的部署对话框会记录您对*权重*许可证的接受，但商业授权是您与供应商之间的直接约束）。对于模式 4，一旦供应商批准您的 HuggingFace 账户，ArtSmoker 现有的受限仓库访问检查就会变绿，部署照常进行。当供应商发布新模型版本时，请重新确认您的授权是否覆盖它（Stability 的会自动覆盖；BFL 的在 Enterprise 之外通常不覆盖；Tencent 每个版本都会发布新的许可证文本）。
...
>
> **模型许可与商业使用**：通过 ArtSmoker 部署的自托管模型受其创作者许可条款约束，这些条款直接约束的是**您**。ArtSmoker 会在部署时展示每个模型的许可证和依赖明细并记录您的接受，但它**不会**验证、执行或保证您的商业使用资格 —— 遵守许可条款（营收/用户数阈值、地域限制、署名要求、使用量报告）完全是您自己的责任。[第 1.9.3 节](#-193-using-a-model-commercially--who-to-pay-and-how)中的商业许可指引仅供参考，反映的是撰写时的供应商条款，**不构成法律建议**；许可条款变化频繁 —— 请务必确认供应商的最新条款，并在商业发布前咨询法律顾问。ArtSmoker 与任何模型供应商均无关联，也不从其获得任何收益。
>
