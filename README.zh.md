...
![ArtSmoker 演示 — 从文本提示词到生产级 2D 美术，再到带完整纹理、为目标引擎量身定制的 3D 模型](docs/images/artsmoker-walkthrough.gif)

## 📌 0. 概述

**ArtSmoker 让一个创意在几分钟内变成专为目标引擎量身定制的美术资产 —— 无需您管理任何流程。** 用自然语言描述一个角色、道具、场景或主视觉，即可获得生产就绪的 2D 美术、完全纹理化的 3D 模型和视频 —— 全部匹配您项目的视觉标识，全部保留在您自己的环境中。最新的 AI 图像、编辑、3D 和视频模型都置于一个简洁、艺术家优先、带有真正创意控制的界面之后：ArtSmoker 为您运行整条生产流程，让您的团队专注于把控外观，而非摆弄底层机器。
...
- **2D、编辑与 3D —— 端到端** —— 先生成，再用局部重绘、扩展绘制、重新着色、搜索替换和参考引导编辑就地精调；将任意 2D 资产转换为**完全纹理化、为目标引擎量身定制的 3D 模型**，可直接放入 Unity、Unreal 或 Blender —— 无需手动建模、UV 展开或纹理绘制。另外还有电影级视频和用于构思的多模型聊天工作室。
...
- 🧊 **图像到 3D 生成** —— 将任何 Game Asset 或 Character 图像一键转换为带纹理的 3D 网格（GLB）。多视图合成 + 纹理烘焙生成为目标引擎量身定制的资产。交互式 3D 查看器支持轨道旋转/缩放/平移
...
*3D 管线的截图 —— 生成、源图审查、面向目标引擎定制的导出与变体 —— 展示在下方第 1.9 节（3D 模型生成）中，与其对应的功能相伴呈现。*
...
从任意 2D 图像生成完全纹理化的 3D 网格 —— 直接在 Asset Viewer 中操作。选择一张 **Game Asset** 或 **Character** 图像，打开 **3D Model** 选项卡，点击 Generate。结果是为目标引擎量身定制的 GLB，您可以对其进行轨道旋转、缩放和下载 —— 无需手动建模、UV 展开或纹理绘制。

**先看最终成果：** ArtSmoker 生成的角色，导出为面向目标引擎定制的 FBX 并在原生 Blender 中打开 —— LOD 链（LOD0–LOD3）完整保留在大纲视图中，纹理已绑定，无需重新绑骨或手工修复。下文将展示如何从一条文本提示词一步步走到这里。
...
![3D 模型查看器 — 各变体工具与面向目标引擎定制的 FBX/USD 导出选项](docs/images/3d-model-viewer-export.png)
