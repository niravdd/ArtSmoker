...
ArtSmoker ist ein selbst gehostetes Kreativstudio, das die besten generativen Modelle von heute hinter einer einzigen künstlerorientierten Oberfläche vereint — speziell für die Produktion von Spiele-Assets konzipiert und ebenso zu Hause in Film, Werbung, E-Commerce, Verlagswesen und jedem Team, das von originären visuellen Inhalten lebt.
...
- **2D, bearbeitet und in 3D — durchgängig** — generieren Sie, verfeinern Sie dann direkt mit Inpainting, Outpainting, Umfärben, Suchen-und-Ersetzen und referenzgeführten Bearbeitungen; verwandeln Sie jedes 2D-Asset in ein **vollständig texturiertes, auf Ihre Spiel-Engine zugeschnittenes 3D-Modell**, das sich direkt in Unity, Unreal oder Blender einfügt — ohne manuelle Modellierung, UV-Unwrapping oder Texture-Painting. Dazu filmische Videos und ein Multi-Modell-Chat-Studio für die Ideenfindung.
...
- 🗂️ **Sammlungen (Set-Generierung)** — mach aus EINEM Prompt **oder einem Referenzbild** ein stimmiges **Set unterschiedlicher Assets** („ein Schachset im Wikinger-Stil", „20 JRPG-Charaktere"). Aktiviere den **Collection**-Schalter im Image Studio — im **Text**-Tab (dein Prompt treibt es) oder im **Image-Inspired**-Tab (der *Look* des Referenzbilds treibt die Art-Direction) — und ein eigener **Collection Designer** zerlegt dein Briefing in eine gemeinsame **Art-Direction** plus ein **Roster** unterschiedlicher, themengerechter Teile, jedes mit eigenem editierbarem, modellunabhängigem Prompt. Bearbeite die Art-Direction, um das ganze Set neu zusammenzusetzen; einzelne Teile sperren, neu generieren, hinzufügen oder löschen; dann alle auf einmal generieren (Optionen × Variationen je Teil), mit Live-Schätzung von Design + Kosten. **Wähle ein Modell für ein einheitliches Set oder mehrere, um jedes Teil auf jedem Modell** zu rendern (modellübergreifender Vergleich). Optionale **hero-** / **reference-anchor**-Kohäsion fixiert den Set-Stil auf ein gewähltes/bereitgestelltes Bild. Sammlungen erscheinen als **eine Karte** in der Gallery; im Sammlungs-Viewer öffnet jedes Teil den normalen Asset Viewer (Optionen / Variationen / Versionen sowie die **Sammlungs-Herkunft** des Teils); du kannst die **repräsentative Version** je Teil anheften, **das ganze Set auf einmal in 3D** wandeln, es als Engine-Bundle **exportieren** oder **die ganze Sammlung zurück ins Image Studio laden**, um sie zu prüfen und neu zu generieren
...
- 🧊 **Image-to-3D-Generierung** — Wandeln Sie jedes Game-Asset- oder Character-Bild mit einem Klick in ein texturiertes 3D-Mesh (GLB) um. Multi-View-Synthese + Texture-Baking erzeugen vollständig texturierte, auf die Spiel-Engine zugeschnittene Assets. Interaktiver 3D-Viewer mit Orbit/Zoom/Schwenk
...
*Die Screenshots der 3D-Pipeline — Generierung, Quellprüfung, auf die Spiel-Engine zugeschnittener Export und Varianten — sind weiter unten in Abschnitt 1.9 (3D-Modell-Generierung) zu sehen, direkt bei den Funktionen, die sie illustrieren.*
...
- **Referenz remixen (Remix the reference)** — klassisches stärkebasiertes Image-to-Image: die *Pixel* Ihrer Referenz gehen direkt in ein Bedrock-Modell (Stable Diffusion 3.5 Large oder Stable Image Ultra — per Registry-Fähigkeitsflag bestimmt), gesteuert über einen **Stärke-Regler**: subtil bewahrt Aufbau, Palette und Stimmung nahezu vollständig; kühn behandelt sie als lose Inspiration. Bei mehr als einer Option entsteht eine **Stärke-Leiter** — eine Karte je Stärke, von subtil bis kühn, nebeneinander. *Bewahrt das Layout, nicht die Identität* (Gesichter und Produkte verändern sich — für identitätsgenaue Arbeit „Getreu der Referenz" verwenden). Die Ausgabegröße folgt dem Referenzbild. **Immer verfügbar** — ohne Bereitstellung und ohne Vision-Analyse-Aufruf.

Alle drei Modi erfordern eine Anweisung, damit Sie die Kontrolle darüber behalten, *wofür* die Referenz dient. Die referenzgeführte Generierung ist getrennt von der Stil-Bibliothek (die viele Bilder zu einem wiederverwendbaren Stilprofil analysiert) — verwenden Sie sie für einmalige, bildgesteuerte Generierungen.
...
**Infrastruktur:** Beide Pipelines werden über denselben 1-Klick-Custom-Models-Flow deployt, wobei der Auswähler zum Deployment-Zeitpunkt für jede Option die Lizenz, die Abhängigkeitstabelle, die Basisinstanz und die geschätzten Kosten/Zeiten anzeigt. Die passgenaue Basisinstanz der vollständigen TRELLIS.2-Pipeline ist **`ml.g5.2xlarge`** (~$1.52/Std.; ein Live-Lauf maß einen Spitzenwert von ~4,78 GB VRAM auf den 24 GB der A10G und wurde sauber abgeschlossen — gewählt gegenüber der L40S-`g6e.xlarge`, die für diese Arbeitslast bei höheren Kosten massiv überdimensioniert ist). Größere `g6e`-Größen bleiben als Reserve-Upsells verfügbar. Endpoints skalieren im Leerlauf auf null — $0 Kosten zwischen Jobs. Der erste Kaltstart baut die CUDA-Erweiterungen einmalig (danach in S3 zwischengespeichert für schnelle Neustarts). Vor dem Deployment eines zugriffsbeschränkten Modells **prüft der Dialog vorab den HuggingFace-Zugriff für jedes Repo, das es herunterlädt** und zeigt pro Repo ein ✓/✗ mit dem genauen nächsten Schritt — sodass Sie nie erst Minuten nach Beginn eines Kaltstarts eine fehlende Lizenzzustimmung entdecken.
...
![3D-Viewer mit Werkzeugen je Variante und auf die Spiel-Engine zugeschnittenen FBX/USD-Exportoptionen](docs/images/3d-model-viewer-export.png)
...
### 📝 1.9.3 Ein Modell kommerziell nutzen — an wen zahlen, und wie

Wenn Ihnen die Ergebnisse eines Modells gefallen und Sie es kommerziell nutzen möchten, hängt der Weg davon ab, **wie sein Hersteller monetarisiert**. ArtSmoker zeigt die Lizenz jedes Modells beim Deployment an; dieser Abschnitt ist der „Was mache ich als Nächstes?"-Begleiter dazu. *(Geprüft anhand der Anbieter-Websites und HuggingFace-Lizenzdateien im August 2026 — Lizenzbedingungen ändern sich schnell, bestätigen Sie daher immer die aktuellen Bedingungen des Anbieters. Nur zur Information — siehe den [Haftungsausschluss](#disclaimer).)*

Die vier Muster, denen Sie begegnen werden:

1. **Bereits Ihres (Apache-2.0 / MIT)** — kommerzielle Nutzung ist inbegriffen, kostenlos. Es gibt kein Lizenzprodukt zu kaufen; der Hersteller monetarisiert stattdessen über seine eigene gehostete API. Ihre einzige Pflicht ist die Einhaltung der Hinweis-/Attributionsanforderungen.
2. **Kostenlos, bis Sie groß sind (Community-Lizenzen)** — kommerzielle Nutzung ist **unterhalb einer Schwelle** inbegriffen (Umsatz oder monatlich aktive Nutzer). Darüber weist Sie die Lizenz selbst an, eine Enterprise-Gewährung beim Anbieter anzufragen — ein Vertriebsgespräch, kein Shop.
3. **Lizenz kaufen, Weights behalten** — die HuggingFace-Weights sind nicht-kommerziell, aber der Hersteller verkauft eine separate **kommerzielle Self-Hosting-Lizenz**. Sobald Sie diese besitzen, werden die *selben Weights, die Sie bereits deployt haben*, für die kommerzielle Nutzung legal — technisch ändert sich in ArtSmoker nichts.
4. **Das Gate ist die Paywall** — das HuggingFace-Repo selbst ist zugangsbeschränkt (gated); eine kommerzielle Vereinbarung mit dem Anbieter schaltet den Zugriff Ihres HF-Kontos frei. Der bestehende HuggingFace-Token- und Gated-Repo-Vorprüfungs-Workflow von ArtSmoker funktioniert dann unverändert.

| Hersteller / Modelle | Muster | Ihr nächster Schritt für kommerzielles Self-Hosting |
|---|---|---|
| **Alibaba** — Qwen-Image, Qwen-Image-Edit | 1 | Nichts zu kaufen (Apache-2.0). Bewahren Sie die Lizenzhinweise auf. |
| **Microsoft** — TRELLIS.2 · **VAST** — TripoSG | 1 | Nichts zu kaufen (MIT). Hinweis: Upstream-Abhängigkeiten (z. B. Meta DINOv3) haben eigene Gates/Bedingungen. |
| **Black Forest Labs** — FLUX.2 [klein] 4B | 1 | Nichts zu kaufen — Apache-2.0, freie kommerzielle Nutzung. |
| **Stability AI** — SD 3.5 (selbst gehostet) | 2 | Kostenlos inkl. kommerzieller Nutzung unter **1 Mio. $ Gesamtjahresumsatz** (die Annahme des HF-Gates *ist* die Lizenz). Darüber **erlischt die Lizenz automatisch** — fordern Sie eine Enterprise-Lizenz unter stability.ai/enterprise an. **Die Attribution „Powered by Stability AI" ist auf jeder Stufe verpflichtend.** |
| **Tencent** — HunyuanImage 3.0 / Hunyuan3D | 2 | Kostenlos inkl. kommerzieller Nutzung unterhalb der Schwelle — **Achtung: sie gilt pro Modell**: HunyuanImage 3.0 = 100 Mio. MAU; **Hunyuan3D-2.1 = nur 1 Mio. MAU** (darüber: E-Mail an hunyuan3d@tencent.com). **Keinerlei Gewährung in der EU, im Vereinigten Königreich oder in Südkorea**, unabhängig von der Größe. |
| **Black Forest Labs** — FLUX.1/.2 [dev], Kontext | 3 | Kaufen Sie eine **FLUX Commercial Weights License** (Self-Service unter dashboard.bfl.ai/licensing; die Stufen sind Abonnements mit Bildvolumen-Obergrenzen). Sie nutzen weiterhin dieselben HF-Weights. Beachten Sie die Pflichten: Nutzungsberichte, Ausgabefilterung, **kein Bereitstellen des Modells als API / kein Weiterverkauf**; neue Modellversionen sind außerhalb von Enterprise **nicht** automatisch abgedeckt. |
| **Bria** — FIBO, RMBG-2.0 | 4 | Das HF-Gate gewährt sofort **nicht-kommerziellen** Zugriff; kommerzielles Self-Hosting erfordert eine **kostenpflichtige Vereinbarung mit Bria** (Kaufformular auf jeder Modellkarte / bria.ai verlinkt). Eine kostenlose kommerzielle Schwelle existiert nicht. Nach der Gewährung deployen Sie über ArtSmoker genau wie zuvor. |

**Wie das mit ArtSmoker zusammenspielt:** Die Beschaffung ändert fast nie etwas Technisches. Bei den Mustern 1–3 sind die Weights, die Sie deployen, vorher und nachher identisch — was sich ändert, ist der Vertrag, den Sie halten (bewahren Sie Ihren Lizenznachweis auf; der Deploy-Dialog von ArtSmoker protokolliert Ihre Annahme der *Weights*-Lizenz, aber kommerzielle Gewährungen binden Sie direkt gegenüber dem Anbieter). Bei Muster 4 wird, sobald der Anbieter Ihr HuggingFace-Konto freischaltet, die bestehende Gated-Repo-Zugriffsprüfung von ArtSmoker grün, und das Deployment läuft normal weiter. Wenn ein Anbieter eine neue Modellversion veröffentlicht, prüfen Sie erneut, ob Ihre Gewährung sie abdeckt (bei Stability automatisch; bei BFL außerhalb von Enterprise in der Regel nicht; Tencent gibt für jede Version einen neuen Lizenztext heraus).
...
# Test 3: Können Sie die Converse-API verwenden? (autorisiert über bedrock:InvokeModel)
...
| `bedrock:InvokeModelWithResponseStream` | Streaming-LLM-Antworten (Chat Studio) — die ConverseStream-API autorisiert sich über diese Aktion. Die Nicht-Streaming-Converse-API (Prompt-Verfeinerung, Stilanalyse, Konzeptgenerierung) autorisiert sich über `bedrock:InvokeModel` — eine separate `bedrock:Converse`-Aktion existiert nicht |
...
| Custom-Models-Laufzeit-Set: `application-autoscaling:*` (Ziele/Richtlinien), `cloudwatch:PutMetricAlarm`/`DeleteAlarms`/`DescribeAlarms`, `logs:` (Lesen + Aufbewahrung), `servicequotas:GetServiceQuota`/`RequestServiceQuotaIncrease`, `ecr:DescribeRepositories`, `iam:CreateServiceLinkedRole` (nur beim allerersten Auto-Scaling) | Endpunkt-Skalierung auf/von null, Backlog-Alarm, Bereitschafts-Scan, GPU-Kontingentprüfung, DLC-Image-Auflösung (optional — nur Custom Models; vollständige Liste in der eingegrenzten Richtlinie unten) |
...
> [!NOTE]
> **Zwei Punkte für Ihr Konto anpassen:** (1) Das S3-Statement ist auf Buckets mit dem Namen `artsmoker-*` begrenzt — wenn Sie ArtSmoker auf einen anders benannten Bucket zeigen lassen (Video Settings erlaubt jeden vorhandenen Bucket), erweitern Sie die `Resource` auf die ARN Ihres Buckets. (2) Von ArtSmoker erzeugte SageMaker-Ressourcen heißen `artsmoker-*`, daher funktioniert die eingegrenzte `Resource` unverändert; `sagemaker:List*`-Aktionen unterstützen keine Ressourcen-Eingrenzung und stehen in einem eigenen Statement.
>
> **Fehlt zur Laufzeit eine Berechtigung?** ArtSmoker überwacht jeden AWS-Aufruf: Wird einer abgelehnt, protokolliert es die exakte fehlgeschlagene `service:Operation` und zeigt eine dauerhafte In-App-Benachrichtigung mit der hinzuzufügenden Aktion — eine Berechtigungslücke schlägt nie stillschweigend fehl.
...
>
> **Modell-Lizenzierung & kommerzielle Nutzung**: Über ArtSmoker deployte selbst gehostete Modelle unterliegen den Lizenzbedingungen ihrer Hersteller, die **Sie** direkt binden. ArtSmoker zeigt beim Deployment die Lizenz und die Abhängigkeits-Aufschlüsselung jedes Modells an und protokolliert Ihre Annahme, es prüft, erzwingt oder garantiert Ihre kommerzielle Berechtigung jedoch **nicht** — die Einhaltung der Lizenzbedingungen (Umsatz-/Nutzerschwellen, territoriale Beschränkungen, Attributionspflichten, Nutzungsberichte) liegt allein in Ihrer Verantwortung. Die Hinweise zur kommerziellen Lizenzierung in [Abschnitt 1.9.3](#-193-ein-modell-kommerziell-nutzen--an-wen-zahlen-und-wie) dienen nur der Information, geben die Anbieterbedingungen zum Zeitpunkt der Erstellung wieder und sind **keine Rechtsberatung**; Lizenzbedingungen ändern sich häufig — bestätigen Sie stets die aktuellen Bedingungen des Anbieters und ziehen Sie vor einem kommerziellen Launch rechtlichen Rat hinzu. ArtSmoker steht in keiner Verbindung zu irgendeinem Modellanbieter und erhält von keinem eine Vergütung.
>
