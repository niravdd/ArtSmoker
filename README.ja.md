...
- 🗂️ **コレクション（セット生成）** — 1つのプロンプト、**または参照画像**から、一貫性のある**個別アセットのセット**（「バイキング風のチェスセット」「20体のJRPGキャラクター」など）を作成します。Image Studio で **Collection** トグルをオンにし、**Text** タブ（プロンプトが駆動）または **Image-Inspired** タブ（参照画像の*見た目*がアートディレクションを駆動）から開始。専用の **Collection Designer** が、共有の**アートディレクション**と、テーマに沿った個別ピースの**ロスター**（各ピースに編集可能なモデル非依存プロンプト付き）へと分解します。アートディレクションを編集してセット全体を再構成、個々のピースをロック／再生成／追加／削除し、すべてをまとめて生成（各オプション×バリエーション）。設計＋予測コストの見積もりをライブ表示。**1モデルで統一感のあるセット、または複数モデルで各ピースをそれぞれのモデルで生成**（モデル横断比較）できます。任意の **hero-** / **reference-anchor** コヒージョンで、選択／提供した画像にセットをスタイル固定。コレクションは Gallery に**1枚のカード**として表示され、Collection ビューアーを開くと各ピースが通常の Asset Viewer（オプション／バリエーション／バージョン、さらにそのピースの**コレクション系譜**）へドリルダウン。各ピースの**代表バージョンを固定**、**セット全体を一括3D化**、**エンジン別バンドルとしてエクスポート**、または**コレクション全体を Image Studio に再読み込み**して確認・再生成できます
