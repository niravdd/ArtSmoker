...
Amazon Bedrockの画像・動画生成モデルのための、シンプルでアーティストフレンドリーなインターフェース。ArtSmokerは、API、CLI、プロンプトエンジニアリングを学ぶことなく、クリエイティブチームがBedrockを効率的に活用できるよう支援します。

### 📝 課題

クリエイティブチームやゲームスタジオはアセット生成にAIを活用したいと考えていますが、実際には次のような障壁に直面します：

- **シンプルなインターフェースがない** — アーティストが画像を生成するために、Bedrockコンソールにログインしたり、APIコールを書いたりする必要があるべきではありません
- **プロンプトエンジニアリングは難しい** — 適切なネガティブプロンプト、スタイル指示、モデル固有のフォーマットを備えた効果的なプロンプトの作成には、ほとんどのアーティストが持ち合わせていない専門知識が必要です
- **チームは独自のモデルを構築/トレーニングしない** — 実際に使えるものを通じて、Bedrockで既に利用可能な多数のモデルにアクセスする必要があります
- **画像編集がアクセスしにくい** — インペインティング、アウトペインティング、検索＆置換、スタイル転送は、いずれもAPIの知識を必要とします
- **2Dから3Dへの変換は別のパイプライン** — 2Dコンセプトからゲームエンジン対応のテクスチャ付き3Dモデルを得るには、通常、手動モデリング、UV展開、テクスチャペイントが必要です — もしくは高額なサードパーティツールが必要になります

### 📝 ソリューション

ArtSmokerは、Amazon Bedrockをクリーンなクリエイティブインターフェースでラップするセルフホスト型Webアプリケーションです — ゲームアセット制作に特化して設計されており、AI生成のビジュアルコンテンツが価値を持つ広告、Eコマース、出版、デジタルメディアなど、他のクリエイティブ産業にも幅広く適用できます。

- **アーティストは必要なものを自然言語で説明するだけ** — ArtSmokerがプロンプトの分解、強化、モデル固有の最適化、スタイルの適用を裏側で処理します。ガイド付きPrompt Designerでは、個々のビジュアル要素（被写体、シーン、照明、色）をロック/バリエーションコントロールで微調整でき、真に異なるクリエイティブオプションを生成できます
- **スタイル認識生成** — ゲームの既存アートをアップロードすると、ArtSmokerのビジョンモデルがビジュアルアイデンティティを学習します。生成されるすべてのアセットが、ゲームのルック＆フィールに合致します
- **すべてのBedrockモデル、すべてのリージョン** — 完全に設定可能。テキストから画像へのモデル、動画モデル、リージョンを選択できます。システムはBedrock APIを通じて利用可能なモデルを動的に検出します
- **セルフホスト型オープンソースモデル — ワンクリックデプロイ** — テスト済みモデルのキュレーションされたカタログ（Qwen-Image、HunyuanImage 3.0、FLUX.2など）を閲覧し、GPUインスタンスを選択して、ワンクリックでAmazon SageMakerにデプロイ。推論のパッケージング、量子化、CUDA設定、オートスケーリング、ジョブトラッキングまで、すべてが処理されます。カタログのすべてのモデルは、出荷前にエンドツーエンドで検証されています
- **ワンクリックで画像から3Dへ** — 任意の2Dゲームアセットやキャラクター画像から、テクスチャ付き3Dモデル（GLB）を直接生成。マルチビュー合成とテクスチャベイキングにより、Unity、Unreal、Blenderに直接インポートできるゲームエンジン対応メッシュを生成します — 手動モデリングは不要です
- **あなたのAWSアカウント、あなたのIP** — すべてがあなた専用のプライベートAWSアカウント内で実行されます。すべてのアートワーク、プロンプト、スタイル、生成されたアセットは、あなたの隔離された環境内に留まります — データがサードパーティサービスに流出することはありません。クリエイティブIPの完全な所有権とコントロールを保持できます
...
- 💰 **コスト追跡** — リクエスト、セッション、アセットごとの推定AWS支出 — PulseBoardテレメトリーに送信
...
任意の2D画像から、プロダクション対応で完全にテクスチャリングされた3Dメッシュを生成します — Asset Viewer内で直接。**Game Asset**または**Character**画像を選択し、**3D Model**タブを開いてGenerateをクリックします。結果は、オービット・ズーム・ダウンロードが可能な、ゲームエンジン対応のGLBです — 手動モデリング、UV展開、テクスチャペイントは一切不要です。
