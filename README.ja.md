...
![ArtSmoker ウォークスルー — テキストプロンプトから、本番品質の2Dアート、そして完全にテクスチャリングされ、ゲームエンジン向けに調整された3Dモデルまで](docs/images/artsmoker-walkthrough.gif)

## 📌 0. 概要

**ArtSmokerは、アイデアをゲームエンジン向けに調整されたアートへと変えます — 数分で、あなたが管理すべきパイプラインは一切なしに。** キャラクター、小道具、環境、キーアートを自然言語で説明するだけで、プロダクション対応の2Dアート、完全にテクスチャリングされた3Dモデル、動画が得られます — すべてがプロジェクトのビジュアルアイデンティティに合致し、すべてがあなた自身の環境内に留まります。最新のAI画像・編集・3D・動画モデルが、本物のクリエイティブコントロールを備えた1つのクリーンでアーティストファーストなインターフェースの背後に収まっています：ArtSmokerが制作パイプライン全体をあなたの代わりに動かすため、チームは機械の操作に追われることなく、ルックの方向づけに専念できます。
...
- **編集と3Dは別世界** — インペインティング、アウトペインティング、リカラー、参照ガイド編集、そして2Dコンセプトをテクスチャ付き3Dモデルに変えることは、通常それぞれに独自のツール、API、専門家を必要とします。
...
- **2Dで、編集し、3Dへ — エンドツーエンド** — 生成し、その場でインペインティング、アウトペインティング、リカラー、検索＆置換、参照ガイド編集で改良できます。任意の2Dアセットを、Unity、Unreal、Blenderに直接ドロップできる**完全にテクスチャリングされ、ゲームエンジン向けに調整された3Dモデル**へと変えます — 手動モデリング、UV展開、テクスチャペイントは不要です。さらに、シネマティックな動画と、アイデア出しのためのマルチモデルチャットスタジオも。
...
3. **真に異なるオプションを取得** — システムは最大5つの明確に異なるクリエイティブコンセプト（衣装、ムード、照明、構図を変化させる — カメラアングルだけではありません）を生成し、それぞれに最大5つのシードバリエーション（合計25画像）を持たせます。ユーザーが指定した詳細はロックされ、AIが推論した詳細は大胆に変化させます。 表示される**シード**コントロールによりバッチは再現可能です — 同じシードに最終プロンプトと設定が揃えば同じ画像を再生成でき、任意の結果をクリックするとそのシードが設定されるため、一つだけ調整してお気に入りから分岐できます。
...
- 🗂️ **コレクション（セット生成）** — 1つのプロンプト、**または参照画像**から、一貫性のある**個別アセットのセット**（「バイキング風のチェスセット」「20体のJRPGキャラクター」など）を作成します。Image Studio で **Collection** トグルをオンにし、**Text** タブ（プロンプトが駆動）または **Image-Inspired** タブ（参照画像の*見た目*がアートディレクションを駆動）から開始。専用の **Collection Designer** が、共有の**アートディレクション**と、テーマに沿った個別ピースの**ロスター**（各ピースに編集可能なモデル非依存プロンプト付き）へと分解します。アートディレクションを編集してセット全体を再構成、個々のピースをロック／再生成／追加／削除し、すべてをまとめて生成（各オプション×バリエーション）。設計＋予測コストの見積もりをライブ表示。**1モデルで統一感のあるセット、または複数モデルで各ピースをそれぞれのモデルで生成**（モデル横断比較）できます。任意の **hero-** / **reference-anchor** コヒージョンで、選択／提供した画像にセットをスタイル固定。コレクションは Gallery に**1枚のカード**として表示され、Collection ビューアーを開くと各ピースが通常の Asset Viewer（オプション／バリエーション／バージョン、さらにそのピースの**コレクション系譜**）へドリルダウン。各ピースの**代表バージョンを固定**、**セット全体を一括3D化**、**エンジン別バンドルとしてエクスポート**、または**コレクション全体を Image Studio に再読み込み**して確認・再生成できます。コレクションは **Image Studio 内にも表示**され(同じカード、その場でビューアを開く)、**費用が発生する前に**アセットタイプがプロンプトと照合されます(セットはキャラクターかゲームアセット)— 意図しない形式でセット全体に支払うことはありません
...
- 🧊 **Image-to-3D生成** — 任意のGame AssetまたはCharacter画像を、ワンクリックでテクスチャ付き3Dメッシュ（GLB）に変換。マルチビュー合成＋テクスチャベイキングにより、ゲームエンジン向けに調整されたアセットを生成します。オービット/ズーム/パン操作が可能なインタラクティブ3Dビューア付き
...
- 🔄 **Auto-Update** — 起動時のバージョンゲート付きチェック＋24時間ごとの定期チェック。`git`（checkout）で更新するか、gitのないインストール環境では**tarballのダウンロード＆置き換え**で更新し、その場で再起動（スーパーバイザによる再起動 / gunicornのリロード）するか、ワンクリックの**再起動**を提示します — `data/`や`.env`を上書きすることはありません（`ARTSMOKER_AUTO_UPDATE=false`で無効化）
...
*3Dパイプラインのスクリーンショット — 生成、ソースレビュー、ゲームエンジン向けに調整されたエクスポート、バリアント — は、下のセクション1.9（3Dモデル生成）で、それぞれが示す機能とともに掲載しています。*
...
- **リファレンスをリミックス（Remix the reference）** — 古典的な強度ベースのimage-to-image：リファレンスの*ピクセル*がBedrockモデル（Stable Diffusion 3.5 LargeまたはStable Image Ultra — レジストリの機能フラグで判定）に直接渡され、**強度ダイヤル**で調整します。控えめなら構図・配色・雰囲気をほぼ維持、大胆なら緩やかなインスピレーション扱いに。オプションを2以上にすると**強度ラダー**になり、控えめ→大胆の各強度が1枚ずつ並びます。*レイアウトは保つが、人物・製品そのものは保たない*（同一性が必要ならMatchを）。出力サイズはリファレンス画像に従います。**常に利用可能** — デプロイ不要、ビジョン解析の呼び出しもなし。

3つのモードすべてが指示を必要とし、リファレンスを*何のために*使うかをあなたがコントロールし続けられます。参照ガイド生成は、Style Library（多数の画像を再利用可能なスタイルプロファイルに分析するもの）とは別物です — 単発の画像駆動生成に使います。
...
|------|-------------|---------|-------------------|
| **Photorealistic Image** *(デフォルト)* | 自然な写真のようなフレーミング — 文脈に合った実在の環境の中の被写体。 | 実世界のカメラ視点：アイレベル、ポートレートは浅い被写界深度、風景はワイド。 | 写真用語で指示（ゴールデンアワー、スタジオソフトボックス、焦点距離の感覚）し、肌の質感・布のしわ・風化といった自然な不完全さを含む。イラスト用語やレンダリングエンジンの専門用語は使わない。 |
...
任意の2D画像から、完全にテクスチャリングされた3Dメッシュを生成します — Asset Viewer内で直接。**Game Asset**または**Character**画像を選択し、**3D Model**タブを開いてGenerateをクリックします。結果は、オービット・ズーム・ダウンロードが可能な、ゲームエンジン向けに調整されたGLBです — 手動モデリング、UV展開、テクスチャペイントは一切不要です。

**まず、最終成果から：** ArtSmokerで生成したキャラクターを、ゲームエンジン向けに調整されたFBXとしてエクスポートし、素のBlenderで開いた状態 — LODチェーン（LOD0〜LOD3）はアウトライナーにそのまま残り、テクスチャもバインド済み。リギングのやり直しも手作業の修正も一切なし。以下では、テキストプロンプトからここに到達する方法を順に示します。
...
| **TRELLIS.2（フル）** | 1つのモデルがジオメトリとPBRテクスチャの**両方**を生成（SLAT） | MITのモデル＋ウェイト | ⚠️ **非商用のベイク** — メッシュのエクスポートが`nvdiffrast`（NVIDIA 1-Way Commercial）をハードインポートします。商用利用の前に確認してください | 最もシンプルな経路。プロトタイプと非商用の用途 |

> **商用利用：** **TripoSG + テクスチャバックエンド**の経路が、商用利用に安全なルートです — そのベイクは**Kaolin (Apache-2.0)**を使用します。**フルのTRELLIS.2**パイプラインはモデル自体はMITですが、その`o_voxel`メッシュベイクが`nvdiffrast`（NVIDIA Source Code License、一般ユーザーには非商用）をハードインポートしており、上流にパッチを当てずに差し替えることはできません — したがって、その依存関係をクリアしていない限り、フルパイプラインの出力は非商用として扱ってください。ライセンスとすべての依存関係は、デプロイダイアログと、生成時に再度表示されます。
...
**インフラ：** どちらのパイプラインも同じワンクリックCustom Modelsフローでデプロイし、デプロイ時のピッカーが各オプションのライセンス、依存関係テーブル、インスタンスのベースライン、推定コスト/時間を表示します。完全なTRELLIS.2パイプラインの適正サイズのベースラインは**`ml.g5.2xlarge`**（～$1.52/時間。実測では、A10Gの24 GBのうちVRAMピークは約4.78 GBで、実行はクリーンに完了しました — このワークロードには過剰にオーバープロビジョニングで、かつコストも高いL40Sの`g6e.xlarge`ではなく、こちらを選定しました）。より大きな`g6e`サイズは、余裕（ヘッドルーム）を増やすアップセルとして引き続き利用できます。エンドポイントはアイドル時にゼロにスケールします — ジョブ間のコストは$0。初回のコールドスタート時に一度だけCUDA拡張をビルドします（以降はS3にキャッシュされ、高速に再起動します）。ゲート付きモデルをデプロイする前に、ダイアログが**取得するすべてのリポジトリについてHuggingFaceアクセスを事前チェック**し、リポジトリごとに✓/✗と正確な次のステップを表示します — コールドスタートの数分後にライセンス同意の漏れに気づく、ということがなくなります。
...
![3D Model viewer with per-variant tools and engine-tailored FBX/USD export options](docs/images/3d-model-viewer-export.png)
...
| `bedrock:InvokeModelWithResponseStream` | ストリーミングLLM応答（Chat Studio）— ConverseStream APIはこのアクションで認可されます。非ストリーミングのConverse API（プロンプト精緻化、スタイル分析、コンセプト生成）は`bedrock:InvokeModel`で認可されます — 独立した`bedrock:Converse`アクションは存在しません |
...
| Custom Models ランタイム一式：`application-autoscaling:*`（ターゲット/ポリシー）、`cloudwatch:PutMetricAlarm`/`DeleteAlarms`/`DescribeAlarms`、`logs:`（読み取り＋保持設定）、`servicequotas:GetServiceQuota`/`RequestServiceQuotaIncrease`、`ecr:DescribeRepositories`、`iam:CreateServiceLinkedRole`（初回のオートスケーリングのみ） | エンドポイントのスケールtoゼロ/fromゼロ、待機アラーム、準備状況スキャン、GPUクォータ確認、DLCイメージ解決（オプション — Custom Modelsのみ；下のスコープ付きポリシーに全記載） |
...
> [!NOTE]
> **アカウントに合わせて調整する2点：** (1) S3ステートメントは`artsmoker-*`という名前のバケットにスコープされています — 別名のバケットを使う場合（Video Settingsでは任意の既存バケットを選択可能）、`Resource`をそのバケットのARNに広げてください。(2) ArtSmokerが作成するSageMakerリソースは`artsmoker-*`という名前なのでスコープ付き`Resource`はそのまま機能します。`sagemaker:List*`アクションはリソーススコープに対応していないため、別ステートメントになっています。
>
> **実行時に権限が不足していたら？** ArtSmokerはすべてのAWS呼び出しを監視します。拒否された場合、失敗した`service:Operation`を正確にログに記録し、追加すべきアクションを明記した永続的なアプリ内通知を表示します — 権限不足が黙って失敗することはありません。
...
**自己再起動（オプション、全プラットフォーム対応）。** 上記のコマンドはそのままで動作します。ArtSmokerに**その場で自分自身を再起動**させる — 自動更新やアプリ内の**再起動**ボタンで、再起動の手間なく新しいコードを読み込ませる — には、代わりに組み込みのクロスプラットフォーム対応スーパーバイザの下で起動してください：

```bash
python -m backend.main            # 必要に応じて --host / --port を追加。Ctrl-Cでクリーンに停止します
```

これは**Windowsを含むあらゆるOS**で動作します。スーパーバイザは子プロセスでアプリを実行し、再起動リクエストがあれば再生成します。（gunicornやsystemdのようなサービスマネージャの下で実行しても、同じその場での再起動が得られます — §4.2および§6を参照してください。）
...
**4. 言語を選択** — 英語以外のインターフェースを好む場合は、ナビバーの言語ボタン（EN | 日 | 中 | 한 | हिं | РУ | FR | ES | DE）をクリックしてください。
...
- スタジオ別に整理された**9つのタブ**：
  ...
  - **Custom Models** — セルフホストモデルカタログ：SageMakerエンドポイントのデプロイ・監視・破棄（セクション6.12参照）
  - **Prompt Templates** — 6つのワークフローセクションに整理された28の編集可能なLLM指示プロンプト（セクション4.4参照）
  - **Registry JSON** — 完全なモデルレジストリ用の生のJSONエディタ
  - **Maintenance** — マネージドツールの状態（例：FBX/USDエクスポート用のヘッドレスBlender — パス、バージョン、オンデマンド更新）
...
**フォーマットファミリー**：モデルは、レジストリ（`format_families`）からリクエストテンプレートを読み取る汎用インボーカーを通じて呼び出されます — 生成、編集、後処理、動画のすべてがテンプレート駆動です。新しいBedrock画像モデルの追加には**コード変更が一切不要**：正しいフォーマットファミリーで（自動検出またはAdmin API経由で）登録するだけです。ファミリーの完全なカタログは[SPEC.md](SPEC.md)にあります。
...
| i18n | カスタムt()関数、約1,500キー×9言語、逆引きDOM翻訳 |
...
| `auto_update` | `ARTSMOKER_AUTO_UPDATE` | true | 起動時のバージョンゲート付き更新＋24時間ごとの定期チェック（gitまたはtarball）、その後その場で再起動 |
