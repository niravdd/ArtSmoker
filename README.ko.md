...
1. **지오메트리 추출** — 정류 플로우 트랜스포머(TripoSG, 15억 파라미터, MIT 라이선스)가 SDF(부호 거리장) 표현을 사용하여 단일 2D 이미지를 고충실도 3D 메시로 변환합니다. 메시 밀도는 품질 프리셋에 따라 조정되며(최고 옥트리 해상도에서 최대 약 100만 면), 얼굴과 장비의 디테일을 선명하게 표현합니다.
2. **텍스처링** — 메시는 **배포 시 선택하는 텍스처 백엔드**로 페인팅됩니다. 기본값은 **TRELLIS.2**(Microsoft, MIT)로, SLAT/복셀 조건부 텍스처러가 4096² 아틀라스에서 완전한 PBR 머티리얼(베이스 컬러 + 메탈릭-러프니스 + 알파)을 생성합니다. 상업적 라이선스가 가능하면서도 업계 최고 수준의 독점 텍스처러와 동등한 품질을 제공합니다.
3. **PBR 출력** — 텍스처가 입혀진 메시는 PBR 맵이 내장된 GLB로 내보내져, 모든 최신 엔진에서 물리 기반 렌더링에 바로 사용할 수 있습니다.

**라이선스를 명확하게 보여주며 텍스처 백엔드를 선택하세요.** 이미지-투-3D는 배포별로 선택 가능한 3개의 텍스처러를 지원합니다. 각각은 배포 대화상자에서 **전체 라이선스 및 의존성 내역**을 표시합니다 — 가져오는 모든 모델, 해당 모델의 라이선스, 상업적 사용 가능 여부 또는 게이팅 여부까지 — 그리고 배포 전에 반드시 읽고 동의해야 합니다:

| 백엔드 | 라이선스 | 상업적 사용 | 적합한 용도 |
|--------|----------|-------------|-------------|
...
배경 제거(텍스처링 전 컷아웃 단계)는 기본적으로 **BiRefNet(MIT)** 을 사용합니다 — 완전히 상업적 사용 가능 — 비상업용 대안(RMBG)은 명시적 옵트인으로 제공됩니다. ArtSmoker는 제한된 의존성을 절대 조용히 가져오지 않습니다: 게이팅되거나 비상업용인 모든 것은 이름이 명시되고, 배지가 표시되며, 명시적 동의 뒤에 게이팅됩니다.

**출력:** PBR 텍스처가 내장된 표준 GLB — Unity, Unreal Engine, Blender 및 기타 게임 엔진에 직접 임포트 가능. 인터랙티브 3D 뷰어는 오빗, 줌, 팬을 지원하여 즉시 확인할 수 있으며, **3D Model** 탭은 완전한 출처 추적을 위해 사용된 정확한 모델 및 도구(지오메트리 모델, 텍스처 백엔드, 의존성, 인스턴스, 파라미터)를 나열합니다.

**인프라:** 동일한 원클릭 Custom Models 플로우로 배포하며, 배포 시 백엔드 피커가 각 옵션의 라이선스, 의존성 표, 인스턴스 베이스라인, 추정 비용/시간을 표시합니다. `ml.g6e` GPU 인스턴스에서 실행되며, 유휴 시 제로로 스케일 — 작업 사이 비용 $0. 첫 콜드 스타트에서 CUDA 익스텐션을 한 번 빌드한 후 S3에 캐시되어 이후 빠르게 재시작됩니다.
