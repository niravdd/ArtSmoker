...
![ArtSmoker 워크스루 — 텍스트 프롬프트에서 프로덕션급 2D 아트, 완전히 텍스처링된 게임 엔진 맞춤형 3D 모델까지](docs/images/artsmoker-walkthrough.gif)

## 📌 0. 개요

**ArtSmoker는 아이디어를 게임 엔진 맞춤형 아트로 바꿉니다 — 관리할 파이프라인 없이, 단 몇 분 만에.** 캐릭터, 소품, 환경, 키 아트를 자연어로 설명하기만 하면 프로덕션 대응의 2D 아트, 완전 텍스처링된 3D 모델, 동영상을 얻을 수 있습니다 — 모두 프로젝트의 비주얼 아이덴티티에 맞춰지고, 모두 자체 환경 내에 유지됩니다. 최신 AI 이미지·편집·3D·동영상 모델이 실질적인 크리에이티브 컨트롤을 갖춘 하나의 깔끔한 아티스트 우선 인터페이스 뒤에 자리합니다: ArtSmoker가 전체 제작 파이프라인을 대신 실행하므로, 팀은 기계 장치를 다루는 대신 룩을 디렉팅하는 데 집중합니다.
...
- **2D, 편집, 그리고 3D — 엔드투엔드** — 생성한 뒤 인페인팅, 아웃페인팅, 색상 변경, 검색 및 교체, 레퍼런스 가이드 편집으로 제자리에서 다듬고; 모든 2D 에셋을 Unity, Unreal, Blender에 바로 들어가는 **완전 텍스처링된 게임 엔진 맞춤형 3D 모델**로 변환합니다 — 수동 모델링, UV 언래핑, 텍스처 페인팅이 필요 없습니다. 여기에 영화적 동영상과 아이디어 발상을 위한 멀티 모델 채팅 스튜디오까지.
...
- 🗂️ **컬렉션(세트 생성)** — 하나의 프롬프트, **또는 참조 이미지**로 일관된 **서로 다른 에셋 세트**(「바이킹풍 체스 세트」, 「JRPG 캐릭터 20종」 등)를 만듭니다. Image Studio에서 **Collection** 토글을 켜고 **Text** 탭(프롬프트가 주도) 또는 **Image-Inspired** 탭(참조 이미지의 *룩*이 아트 디렉션을 주도)에서 시작합니다. 전용 **Collection Designer**가 브리프를 공유 **아트 디렉션**과 테마에 맞는 서로 다른 조각들의 **로스터**(각 조각마다 편집 가능한 모델 비의존 프롬프트)로 분해합니다. 아트 디렉션을 편집해 세트 전체를 재구성하고, 개별 조각을 잠그기/재생성/추가/삭제하며, 전부를 한 번에 생성(각 옵션×변형)합니다. 설계+예상 비용을 실시간 표시. **하나의 모델로 통일된 세트를, 또는 여러 모델로 각 조각을 모델별로** 생성(모델 교차 비교)할 수 있습니다. 선택적 **hero-** / **reference-anchor** 응집성은 선택/제공한 이미지에 세트의 스타일을 고정합니다. 컬렉션은 Gallery에 **하나의 카드**로 표시되며, 컬렉션 뷰어를 열면 각 조각이 일반 Asset Viewer(옵션/변형/버전 및 해당 조각의 **컬렉션 계보**)로 드릴다운됩니다. 각 조각의 **대표 버전 고정**, **세트 전체를 한 번에 3D화**, **엔진별 번들로 내보내기**, 또는 **컬렉션 전체를 Image Studio로 다시 불러와** 검토·재생성할 수 있습니다. 컬렉션은 **Image Studio 안에도 표시**되며(같은 카드, 그 자리에서 뷰어 열기), **비용이 발생하기 전에** 에셋 유형을 프롬프트와 대조합니다(세트는 캐릭터 또는 게임 에셋) — 잘못된 형식으로 세트 전체에 비용을 쓰는 일이 없습니다
...
- 🧊 **이미지-투-3D 생성** — 모든 Game Asset 또는 Character 이미지를 원클릭으로 텍스처링된 3D 메시(GLB)로 변환합니다. 멀티뷰 합성 + 텍스처 베이킹으로 게임 엔진 맞춤형 에셋을 생성합니다. 오빗/줌/팬이 가능한 인터랙티브 3D 뷰어
...
*3D 파이프라인의 스크린샷 — 생성, 소스 검토, 게임 엔진 맞춤형 내보내기, 배리언트 — 은 아래 1.9절(3D 모델 생성)에서 해당 기능과 함께 보여드립니다.*
...
임의의 2D 이미지에서 완전 텍스처링된 3D 메시를 생성합니다 — Asset Viewer에서 직접 수행합니다. **Game Asset** 또는 **Character** 이미지를 선택하고, **3D Model** 탭을 열어 Generate를 클릭합니다. 결과물은 게임 엔진 맞춤형 GLB로, 오빗·줌·다운로드가 가능하며 — 수작업 모델링, UV 언래핑, 텍스처 페인팅이 전혀 필요 없습니다.

**최종 결과를 먼저:** ArtSmoker로 생성한 캐릭터를 게임 엔진 맞춤형 FBX로 내보내 순정 Blender에서 연 모습 — LOD 체인(LOD0–LOD3)이 아웃라이너에 그대로 유지되고 텍스처도 바인딩된 상태로, 리깅을 다시 하거나 손으로 고칠 것이 없습니다. 아래에서는 텍스트 프롬프트에서 여기까지 도달하는 방법을 차례로 보여줍니다.
...
| **TRELLIS.2 (Full)** | 하나의 모델이 지오메트리와 PBR 텍스처(SLAT)를 **모두** 생성 | MIT 모델 + 가중치 | ⚠️ **비상업용 베이크** — 메시 내보내기가 `nvdiffrast`(NVIDIA 1-Way Commercial)를 하드 임포트합니다; 상업적 사용 전 검토하세요 | 가장 간단한 경로, 프로토타입 및 비상업용 작업 |

> **상업적 사용:** **TripoSG + 텍스처 백엔드** 경로가 상업적으로 안전한 경로입니다 — 이 경로의 베이크는 **Kaolin (Apache-2.0)** 을 사용합니다. **전체 TRELLIS.2** 파이프라인은 모델 자체는 MIT이지만, 그 `o_voxel` 메시 베이크가 `nvdiffrast`(NVIDIA Source Code License, 일반 사용자에게는 비상업용)를 하드 임포트하며 업스트림을 패치하지 않고는 교체할 수 없습니다 — 따라서 그 의존성을 해결하지 않았다면 전체 파이프라인 출력을 비상업용으로 취급하세요. 라이선스와 모든 의존성은 배포 대화상자에, 그리고 생성 시에 다시 표시됩니다.
...
**인프라:** 두 파이프라인 모두 동일한 원클릭 Custom Models 플로우로 배포하며, 배포 시 피커가 각 옵션의 라이선스, 의존성 표, 인스턴스 베이스라인, 추정 비용/시간을 표시합니다. 전체 TRELLIS.2 파이프라인의 적정 베이스라인은 **`ml.g5.2xlarge`**(~$1.52/시간, 실제 실행에서 A10G의 24 GB 중 피크 ~4.78 GB VRAM을 측정하며 문제없이 완료 — 이 워크로드에 비해 훨씬 과다 프로비저닝되고 비용도 더 높은 L40S `g6e.xlarge` 대신 선택)입니다. 더 큰 `g6e` 크기는 여유 확보용 업셀로 계속 제공됩니다. 엔드포인트는 유휴 시 제로로 스케일됩니다 — 작업 사이 비용 $0. 첫 콜드 스타트에서 CUDA 익스텐션을 한 번 빌드한 후(그다음 빠른 재시작을 위해 S3에 캐시됩니다). 게이팅된 모델을 배포하기 전에, 대화상자가 **모델이 가져오는 모든 repo에 대한 HuggingFace 접근 권한을 사전 점검**하고 repo별로 ✓/✗와 정확한 다음 단계를 표시합니다 — 따라서 콜드 스타트가 몇 분간 진행된 뒤에야 누락된 라이선스 동의를 발견하는 일이 절대 없습니다.
...
![3D 모델 뷰어 — 배리언트별 도구와 게임 엔진 맞춤형 FBX/USD 내보내기 옵션](docs/images/3d-model-viewer-export.png)
