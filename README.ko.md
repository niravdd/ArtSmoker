...
- **2D에서 3D로의 변환은 별도 파이프라인** — 2D 컨셉에서 게임 엔진에 바로 사용할 수 있는 텍스처 입힌 3D 모델을 만들려면 일반적으로 수동 모델링, UV 언래핑, 텍스처 페인팅이 필요합니다 — 또는 비싼 서드파티 도구가 필요합니다
...
- **아티스트는 필요한 것을 자연어로 설명하기만 하면 됩니다** — ArtSmoker가 프롬프트 분해, 강화, 모델별 최적화, 스타일 적용을 뒷단에서 처리합니다. 가이드 Prompt Designer를 통해 개별 비주얼 요소(주체, 씬, 조명, 색상)를 잠금/변형 컨트롤로 미세 조정하여 진정으로 다른 크리에이티브 옵션을 생성할 수 있습니다
...
ArtSmoker는 8개 언어를 지원합니다. 상단 내비게이션 바의 언어 버튼(EN | 日 | 中 | 한 | हिं | РУ | FR | ES)으로 UI 언어를 전환할 수 있습니다. 선택은 자동으로 저장됩니다.
...
1. **필요한 것을 설명** — "hospital building"이나 "fire mage character"와 같은 프롬프트를 입력하거나 음성 입력을 사용합니다. AI가 아이디어를 비주얼 컴포넌트로 분해하고, 모델별 최적화로 강화하며, 스마트 잠금/변형 컨트롤로 크리에이티브 의도를 존중합니다. 어떤 언어로든 입력 가능 — 비영어 프롬프트는 자동 번역됩니다.
2. **모델과 설정 선택** — 사용 가능한 모든 텍스트-이미지 모델(Amazon Bedrock + SageMaker 자체 호스팅)에서 멀티 선택하고, 크기, 품질 티어, 리전을 선택합니다. 여러 모델을 체크하여 나란히 비교하거나, 하나를 선택하여 집중 생성합니다. 비용 추정은 실시간으로 업데이트됩니다.
3. **진정으로 다른 옵션 얻기** — 시스템이 최대 5개의 명확히 다른 크리에이티브 컨셉트(의상, 분위기, 조명, 구도를 변형 — 단순히 카메라 앵글만이 아닌)를 생성하며, 각각 최대 5개의 시드 배리에이션(총 25개 이미지)을 갖습니다. 사용자가 지정한 세부사항은 잠기고, AI가 추론한 세부사항은 과감하게 변형됩니다.
...
- 🖼️ **2D Image Studio** — 옵션 x 배리에이션으로 이미지 생성, 가이드 3단계 프롬프트 워크플로
- 🎨 **Prompt Designer** — AI가 프롬프트를 편집 가능한 시각적 컴포넌트(주체, 씬, 조명, 색상)로 분해, 필드별 잠금/변형 토글, 스타일 통합, 스마트 에셋 타입 분류. Photorealistic, Character, Environment 등
- 🎬 **Video Studio** — 모델별 프롬프트 가이던스(Nova Reel 카메라 컨트롤, Luma Ray 자연어), 멀티숏, 이미지-동영상
...
- 📥 **이미지 가져오기** — 기존 이미지(모든 형식)를 갤러리에 일급 에셋으로 가져옵니다. 자동으로 PNG로 변환하고 선택한 에셋 유형을 지정하며, 편집·버전 관리·이미지-투-3D 등 모든 기능이 생성 이미지와 동일하게 작동합니다
- ✏️ **이미지 편집** — 인페인팅, 아웃페인팅, 지우기, 검색 및 교체, 색상 변경(AssetViewer 내). 각 모드에는 AI **프롬프트 생성** 버튼이 있어 비전 모델이 이미지와 원본 프롬프트를 읽고 해당 모드와 선택한 편집 모델에 맞춘 편집 프롬프트를 제안합니다(Stability 편집 모델에는 설명형 캡션, Qwen-Image-Edit에는 지시문). 확장/아웃페인트는 확정 전에 픽셀 눈금자와 함께 캔버스 확장을 실시간 미리보기로 표시합니다. 지시형 에디터(Qwen-Image-Edit)는 **마스크 없이 5가지 모드 전부**를 지원 — 진정한 캔버스 확장 포함: ArtSmoker가 캔버스를 사전 패딩하고 모델이 새 영역만 완성하게 한 뒤 원본 픽셀을 그대로 블렌딩해 되돌립니다. 편집된 각 버전에는 원본 생성 모델과 해당 버전을 편집한 모델 **두 개의 태그**가 함께 표시됩니다
- 📤 **내보내기 및 컷아웃** — AssetViewer에서 버전별 내보내기 산출물: 배경이 제거된 투명 PNG 컷아웃과 진정한 벡터 SVG 트레이스(배경 포함/미포함). 배경 제거 방식은 실행마다 선택 가능: **무료 온디바이스**(rembg/u2net, 클라우드 비용 없음) 또는 **유료 Amazon Bedrock** 리무버 — 3D 생성용 이미지 준비 시에도 동일한 선택지를 제공
...
- 📦 **에셋 버전 관리** — 버전 기록(v1, v2…)과 버전 탐색을 지원하는 제자리 편집, 그리고 버전별 삭제: 하나의 버전만 제거(다른 버전 번호는 유지), 뷰어는 이전 버전으로 전환 — 마지막 버전을 삭제하면 에셋 전체가 제거됩니다
...
- 🩹 **3D용 스마트 소스 완성** — 이미지-투-3D는 보이는 부분만 생성할 수 있어, 잘린 캐릭터(다리가 잘림)는 다리 없는 메시가 됩니다. 생성 전에 소스 이미지를 비전으로 확인하고, 잘려 있으면 **아웃페인팅으로 완성**(AI가 제안하고 완전히 편집 가능한 프롬프트)을 제안합니다 — 전/후를 미리 보고, 결과를 재검토하며, 다시 확장하거나 버릴 수 있고, 새 이미지 버전으로 저장됩니다. 선택적이며 비차단형이고, 잘 구성된 이미지는 그대로 생성됩니다
...
**갤러리** — 미디어 유형 필터, 스타일 필터, 검색, 정렬이 포함된 생성 이미지와 동영상의 통합 뷰. 에셋을 클릭하면 전체 뷰어가 열립니다. **이미지 가져오기** 버튼으로 기존 이미지를 갤러리에 가져올 수 있습니다 — 에셋 유형을 선택하면(캐릭터／게임 에셋은 3D 지원) PNG로 변환되어 즉시 편집·3D 변환이 가능합니다.
...
1. **지오메트리 추출** — 정류 플로우 트랜스포머(TripoSG, 15억 파라미터, MIT 라이선스)가 SDF(부호 거리장) 표현을 사용하여 단일 2D 이미지를 고충실도 3D 메시로 변환합니다. 메시 밀도는 품질 프리셋에 따라 조정됩니다(최고 옥트리 해상도에서 최대 약 100만 면).
2. **텍스처링** — 메시는 **배포 시 선택하는 텍스처 백엔드**로 페인팅됩니다(기본값 **TRELLIS.2**, Microsoft, MIT — SLAT/복셀 조건부 텍스처러로 4096² 아틀라스에서 완전한 PBR을 생성).
3. **PBR 출력** — PBR 맵이 내장된 GLB로 내보내져, 모든 최신 엔진에서 바로 사용할 수 있습니다.

**TRELLIS.2 (Full)** 파이프라인은 동일한 작업을 단일 모델에서 엔드투엔드로 수행합니다 — 별도의 텍스처링 단계가 없습니다.

**라이선스를 명확하게 — 배포 시 그리고 생성 시에도.** 배포 가능한 각 옵션은 배포 대화상자에서 **전체 라이선스 및 의존성 내역**을 표시합니다 — 가져오는 모든 모델, 해당 모델의 라이선스, 상업적 사용 가능 여부 또는 게이팅 여부까지 — 배포 전에 읽고 동의해야 합니다. 생성 시에는 Asset Viewer가 라이선스를 다시 표시하고 *"`<날짜>`에 배포 시 동의함"* 을 확인해 줍니다(추가 클릭 불필요):
...
**인프라:** 두 파이프라인 모두 동일한 원클릭 Custom Models 플로우로 배포하며, 배포 시 피커가 각 옵션의 라이선스, 의존성 표, 인스턴스 베이스라인, 추정 비용/시간을 표시합니다. 전체 TRELLIS.2 파이프라인의 적정 베이스라인은 **`ml.g6e.xlarge`**(~$2.61/시간, 측정된 피크는 ~6.5 GB VRAM + ~22 GB 호스트 RAM — GPU가 아니라 호스트 RAM이 제약 조건)입니다. 더 큰 `g6e` 크기는 RAM 여유 확보용 업셀로 제공됩니다. 엔드포인트는 유휴 시 제로로 스케일 — 작업 사이 비용 $0. 첫 콜드 스타트에서 CUDA 익스텐션을 한 번 빌드한 후 S3에 캐시됩니다. 게이트된 모델을 배포하기 전에, 대화창이 **모델이 가져오는 모든 repo에 대한 HuggingFace 접근 권한을 사전 점검**하고 repo별로 ✓/✗와 정확한 다음 단계를 표시합니다 — 따라서 콜드 스타트가 몇 분간 진행된 후에야 누락된 라이선스 동의를 발견하는 일이 절대 없습니다. 프런티어 모델(OpenAI GPT-5.x, Claude Mythos, GLM, Grok 등)은 Amazon Bedrock Mantle 엔드포인트를 통해 제공되며, 이를 사용하려면 `AmazonBedrockMantleInferenceAccess` IAM 권한이 필요합니다 — 전체 필수 권한 목록은 영문 README.md를 참조하세요.
...
| i18n | 커스텀 t() 함수, 817개 키 x 8개 언어, 역방향 조회 DOM 번역 |
