...
**인프라:** 두 파이프라인 모두 동일한 원클릭 Custom Models 플로우로 배포하며, 배포 시 피커가 각 옵션의 라이선스, 의존성 표, 인스턴스 베이스라인, 추정 비용/시간을 표시합니다. `ml.g6e` GPU 인스턴스에서 실행되며, 유휴 시 제로로 스케일 — 작업 사이 비용 $0. 첫 콜드 스타트에서 CUDA 익스텐션을 한 번 빌드한 후 S3에 캐시됩니다.
