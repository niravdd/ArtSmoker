...
크리에이티브 팀과 게임 스튜디오는 AI를 활용한 에셋 생성을 원하지만, 실제로는 많은 장벽에 직면합니다:

- **간단한 인터페이스가 없음** — 아티스트가 이미지를 생성하기 위해 Bedrock 콘솔에 로그인하거나 API 호출을 작성해야 할 필요는 없어야 합니다
- **프롬프트 엔지니어링이 어려움** — 적절한 네거티브 프롬프트, 스타일 지시, 모델별 포맷을 갖춘 효과적인 프롬프트 작성에는 대부분의 아티스트가 갖추지 못한 전문 지식이 필요합니다
...
- **2D-투-3D는 별도의 파이프라인** — 2D 컨셉에서 게임 엔진에 바로 사용할 수 있는 텍스처링된 3D 모델을 만들려면 일반적으로 수동 모델링, UV 언래핑, 텍스처 페인팅이 필요합니다 — 또는 비싼 서드파티 도구가 필요합니다
...
- **아티스트는 필요한 것을 자연어로 설명하기만 하면 됩니다** — ArtSmoker가 프롬프트 분해, 강화, 모델별 최적화, 스타일 적용을 뒷단에서 처리합니다. 가이드형 Prompt Designer를 통해 개별 시각 요소(주체, 씬, 조명, 색상)를 잠금/변형 컨트롤로 미세 조정하여 진정으로 구별되는 크리에이티브 옵션을 만들 수 있습니다
...
- **셀프 호스팅 오픈소스 모델 — 원클릭 배포** — 사전 테스트된 모델(Qwen-Image, HunyuanImage 3.0, FLUX.2 등)의 큐레이션 카탈로그를 탐색하고, GPU 인스턴스를 선택한 후 원클릭으로 Amazon SageMaker에 배포합니다. 추론 패키징, 양자화, CUDA 구성, 오토 스케일링, 작업 추적까지 모든 것이 처리됩니다. 카탈로그의 모든 모델은 출시 전 엔드투엔드로 검증되었습니다
- **원클릭 이미지-투-3D** — 모든 2D 게임 에셋이나 캐릭터 이미지에서 텍스처링된 3D 모델(GLB)을 직접 생성합니다. 멀티뷰 합성과 텍스처 베이킹으로 Unity, Unreal, Blender에 바로 임포트되는 게임 엔진용 메시를 제작합니다 — 수동 모델링 불필요
...
3. **진정으로 다른 옵션 얻기** — 시스템이 최대 5개의 명확히 구별되는 크리에이티브 컨셉(의상, 분위기, 조명, 구도를 변형 — 단순히 카메라 앵글만이 아닌)을 생성하며, 각각 최대 5개의 시드 배리에이션(총 25개 이미지)을 갖습니다. 사용자가 지정한 세부사항은 잠기고, AI가 추론한 세부사항은 과감하게 변형됩니다.
...
두 모드 모두 지시문이 필요하므로, 레퍼런스가 *무엇을 위한 것인지*에 대한 통제권을 유지할 수 있습니다. 레퍼런스 가이드 생성은 Style Library(여러 이미지를 재사용 가능한 스타일 프로필로 분석)와는 별개입니다 — 일회성, 이미지 주도 생성에 사용하세요.
...
임의의 2D 이미지에서 프로덕션 대응의 완전 텍스처링된 3D 메시를 생성합니다 — Asset Viewer에서 직접 수행합니다. **Game Asset** 또는 **Character** 이미지를 선택하고, **3D Model** 탭을 열어 Generate를 클릭합니다. 결과물은 게임 엔진에 바로 사용할 수 있는 GLB로, 오빗·줌·다운로드가 가능하며 — 수작업 모델링, UV 언래핑, 텍스처 페인팅이 전혀 필요 없습니다.
...
| `bedrock:Converse` | LLM 호출 — 프롬프트 정제, 스타일 분석, 컨셉 생성 |
