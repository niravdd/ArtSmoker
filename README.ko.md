...
2. **모델과 설정 선택** — Amazon Bedrock에서 사용 가능한 모든 텍스트-이미지 모델 중에서 선택하고, 크기, 품질 티어, 리전을 선택합니다. 또는 "All Available Models"를 사용하여 활성화된 모든 모델에서 동시에 생성하여 나란히 비교할 수 있습니다.
...
- 🖼️ **2D Image Studio** — 옵션 x 배리에이션으로 이미지 생성, 2영역 프롬프트 에디터
...
- 📝 **Prompt Templates** — 14개의 편집 가능한 LLM 지시 프롬프트, AI 보조 개선, 변수 검증과 자동 수정
...
**2D Image Studio** — 왼쪽에 설정, 오른쪽에 AI 강화 프롬프트, 하단에 모델 비교 결과. All Available Models 모드는 활성화된 모든 이미지 모델에서 동시에 생성합니다.
...
모델 드롭다운에서 **"All Available Models"**를 선택하면 활성화된 모든 이미지 모델에서 프롬프트를 동시에 생성합니다 — 모델당 1개 이미지. Nova Canvas, Titan Image, SD 3.5 Large, Stable Image Ultra가 동일한 프롬프트를 각각 어떻게 해석하는지 직접 비교할 수 있습니다. 각 모델은 독립적으로 실행됩니다: 엄격한 모델이 프롬프트를 차단해도 수락한 모델의 결과는 받을 수 있으며, 각 옵션 카드에 명확한 상태 레이블(성공, 콘텐츠 검토에 의해 차단, 또는 실패)이 표시됩니다.

선택적 **"Model-optimized prompts"** 토글은 동일한 프롬프트를 모두에게 보내는 대신 각 모델의 강점에 맞게 프롬프트를 조정합니다 — 직접 비교보다 각 모델에서 최상의 출력을 얻고 싶을 때 유용합니다.
