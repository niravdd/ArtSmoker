...
- **2D, editado y en 3D — de extremo a extremo** — genere y luego refine in situ con inpainting, outpainting, recoloreado, búsqueda y reemplazo, y ediciones guiadas por referencia; convierta cualquier recurso 2D en un **modelo 3D completamente texturizado y adaptado al motor de juego** que se integra directamente en Unity, Unreal o Blender — sin modelado manual, desenvolvimiento UV ni pintado de texturas. Además, video cinematográfico y un chat studio multimodelo para la ideación.
...
- 🗂️ **Colecciones (generación de conjuntos)** — convierte UN prompt **o una imagen de referencia** en un **conjunto coherente de recursos distintos** («un juego de ajedrez de estilo vikingo», «20 personajes JRPG»). Activa el interruptor **Collection** en el Image Studio — desde la pestaña **Text** (lo impulsa tu prompt) o **Image-Inspired** (el *aspecto* de la imagen de referencia impulsa la dirección de arte) — y un **Collection Designer** dedicado descompone tu brief en una **dirección de arte** compartida más un **roster** de piezas distintas en la temática, cada una con su propio prompt editable e independiente del modelo. Edita la dirección de arte para recomponer todo el conjunto; bloquea, regenera, añade o elimina piezas individuales; luego genéralas todas de una vez (opciones × variaciones cada una), con una estimación en vivo del diseño y el coste proyectado. **Elige un solo modelo para un conjunto homogéneo, o varios para renderizar cada pieza en cada uno** (comparación entre modelos). La cohesión opcional **hero-** / **reference-anchor** fija el estilo del conjunto a una imagen elegida/proporcionada. Las colecciones aparecen como **una sola tarjeta** en la Gallery; el visor de colección permite que cada pieza abra el Asset Viewer normal (opciones / variaciones / versiones, más el **linaje de colección** de la pieza); puedes **fijar la versión** que representa cada pieza, **pasar todo el conjunto a 3D** de una vez, **exportarlo** como paquete por motor, o **recargar toda la colección en el Image Studio** para revisarla y regenerarla. Tus colecciones también aparecen **dentro del Image Studio** (las mismas tarjetas, abriendo el visor en el sitio) y, **antes de gastar nada**, se comprueba el tipo de recurso frente a tu prompt (un conjunto = Personajes o Recursos de juego), para que nunca pagues un conjunto entero con el encuadre equivocado
...
- 🧊 **Generación de imagen a 3D** — Convierta cualquier imagen de Game Asset o Character en una malla 3D texturizada (GLB) con un clic. La síntesis multivista + el baking de texturas producen recursos adaptados al motor de juego. Visor 3D interactivo con órbita/zoom/panorámica
...
| **TRELLIS.2 (Full)** | Un solo modelo genera **tanto** la geometría como la textura PBR (SLAT) | modelo + pesos MIT | ⚠️ **Horneado no comercial** — la exportación de la malla importa de forma rígida `nvdiffrast` (NVIDIA 1-Way Commercial); revise antes de un uso comercial | El camino más simple; prototipos y trabajo no comercial |

> **Uso comercial:** la ruta **TripoSG + backend de texturizado** es la vía segura para uso comercial — su horneado usa **Kaolin (Apache-2.0)**. El pipeline **completo de TRELLIS.2** es MIT en cuanto al modelo en sí, pero su horneado de malla `o_voxel` importa de forma rígida `nvdiffrast` (NVIDIA Source Code License, no comercial para usuarios generales) y no puede sustituirse sin parchear el upstream — así que trate la salida del pipeline completo como no comercial a menos que haya resuelto esa dependencia. La licencia + cada dependencia se muestran en el diálogo de despliegue y de nuevo en el momento de generar.
