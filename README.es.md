...
Una interfaz sencilla y orientada al artista para los modelos de generación de imágenes y video de Amazon Bedrock. ArtSmoker ayuda a los equipos creativos a usar Bedrock de forma eficiente — sin necesidad de aprender la API, la CLI ni la ingeniería de prompts.

### 📝 El problema

Los equipos creativos y los estudios de videojuegos quieren usar IA para la generación de recursos, pero enfrentan barreras reales:

- **No hay una interfaz sencilla** — los artistas no deberían tener que iniciar sesión en la consola de Bedrock ni escribir llamadas a la API para generar imágenes
- **La ingeniería de prompts es difícil** — componer prompts eficaces con prompts negativos adecuados, directivas de estilo y formato específico de cada modelo requiere una experiencia que la mayoría de los artistas no tiene
- **Los equipos no construyen/entrenan sus propios modelos** — necesitan acceso a los numerosos modelos ya disponibles en Bedrock, a través de algo que realmente puedan usar
- **La edición de imágenes es inaccesible** — inpainting, outpainting, búsqueda y reemplazo, y transferencia de estilo requieren todos conocimiento de la API
- **El paso de 2D a 3D es un pipeline aparte** — pasar de un concepto 2D a un modelo 3D texturizado listo para el motor de juego normalmente requiere modelado manual, desenvolvimiento UV y pintado de texturas — o herramientas de terceros costosas

### 📝 La solución

ArtSmoker es una aplicación web autoalojada que envuelve Amazon Bedrock en una interfaz creativa y limpia — creada específicamente para la producción de recursos de videojuegos, con aplicabilidad en otras industrias creativas como la publicidad, el comercio electrónico, la edición y los medios digitales, donde el contenido visual generado por IA aporta valor.

- **Los artistas describen lo que necesitan** en lenguaje natural — ArtSmoker se encarga de la descomposición del prompt, la mejora, la optimización específica de cada modelo y la aplicación del estilo entre bastidores. Un Prompt Designer guiado permite a los usuarios ajustar elementos visuales individuales (sujeto, escena, iluminación, colores) con controles de bloquear/variar para opciones creativas genuinamente distintas
...
- **Modelos de código abierto autoalojados — despliegue en 1 clic** — explore un catálogo curado de modelos preprobados (Qwen-Image, HunyuanImage 3.0, FLUX.2 y más), elija una instancia de GPU y despliegue en Amazon SageMaker con un clic. Todo se gestiona automáticamente: empaquetado de la inferencia, cuantización, configuración de CUDA, escalado automático y seguimiento de trabajos. Cada modelo del catálogo se valida de extremo a extremo antes de su publicación
- **Imagen a 3D en un clic** — genere un modelo 3D texturizado (GLB) directamente desde cualquier recurso 2D de juego o imagen de personaje. La síntesis multivista y el baking de texturas producen mallas listas para el motor de juego que se importan directamente en Unity, Unreal o Blender — sin modelado manual
- **Su cuenta de AWS, su IP** — todo se ejecuta en su propia cuenta privada de AWS. Todas las obras, prompts, estilos y recursos generados permanecen dentro de su entorno aislado — ningún dato sale hacia servicios de terceros. Usted mantiene la propiedad y el control totales de su IP creativa
...
- 🔄 **Auto-Update** — Git pull con control de versión al inicio, auto-reinicio tras la actualización, verificación periódica cada 24h (`ARTSMOKER_AUTO_UPDATE=false` para desactivar)
...
**2D Image Studio — Comparación de modelos** — Cuadrícula de comparación lado a lado de todos los modelos seleccionados (7 modelos mostrados). Las variaciones de la opción seleccionada se muestran abajo. Interruptores de postprocesamiento a la izquierda (Remove Background, Convert to SVG, Upscale).
...
**Asset Viewer** — Vista previa a tamaño completo con interfaz de pestañas (PNG, Edit, SVG, Metadata, 3D Model). Descarga directa de PNG y SVG. La composición de fondo transparente se muestra con un patrón de tablero de ajedrez.

![Asset Viewer — Vista previa a tamaño completo con opciones de descarga](docs/images/asset-viewer.png)

**Asset Viewer — Edición de imágenes** — Pestaña Edit con inpainting: pinte una máscara sobre el área a cambiar, describa lo que desea, seleccione un modelo de edición y aplique. El historial de versiones se conserva — los originales nunca se sobrescriben.

![Asset Viewer — Inpainting con máscara y prompt](docs/images/asset-viewer-edit.png)

**Generación de modelo 3D** — Convierta cualquier imagen de Game Asset o Character en una malla 3D texturizada (GLB). Configure la resolución de marching cubes, la proporción de primer plano y los parámetros de generación directamente en la pestaña 3D Model del Asset Viewer.
...
Ambos modos requieren una instrucción para que usted mantenga el control de *para qué* sirve la referencia. La generación guiada por referencia es independiente de la Style Library (que analiza muchas imágenes en un perfil de estilo reutilizable) — úsela para generaciones puntuales impulsadas por imágenes.
...
| `bedrock:Converse` | Llamadas a LLM — refinamiento de prompts, análisis de estilo, generación de conceptos |
...
**4. Elija su idioma** — Haga clic en un botón de idioma en la barra de navegación (EN | JA | ZH | KO | FR | ES) si prefiere una interfaz que no esté en inglés.
...
- **7 pestañas** organizadas por estudio:
...
**Familias de formato**: Los modelos se invocan a través de un invocador genérico (`invoke_image_model`) que lee las plantillas de solicitud del registro (`format_families`). Actualmente 15 familias que cubren generación de imágenes (2), edición de imágenes (8), postprocesamiento (2) y generación de video (2):
- **Generación de imágenes**: `stability_text_to_image` (SD 3.5 Large, Stable Image Ultra, Stable Image Core), además de familias autoalojadas (`sagemaker_*`) para FLUX, HunyuanImage y Qwen-Image
- **Edición de imágenes**: `amazon_inpainting`, `amazon_outpainting`, `stability_inpaint`, `stability_outpaint`, `stability_erase`, `stability_search_replace`, `stability_search_recolor`, `stability_control`, `stability_style_transfer`
- **Postprocesamiento**: `stability_remove_bg`, `stability_upscale`
- **Video**: `nova_reel`, `luma_ray`

Añadir un nuevo modelo de imagen de Bedrock no requiere ningún cambio de código — simplemente regístrelo vía la admin API o el autodescubrimiento con la familia de formato correcta.
...
| i18n | Función personalizada t(), 817 claves × 6 idiomas, traducción del DOM por búsqueda inversa |
...
| `auto_update` | `ARTSMOKER_AUTO_UPDATE` | true | Git pull al inicio + verificación periódica cada 24h, auto-reinicio tras la actualización |
