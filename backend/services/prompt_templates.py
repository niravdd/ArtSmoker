...
Your task: produce {num_options} enhanced text-to-image prompt(s) that realize the user's instruction while staying faithful to the core subject shown in the reference image(s). When more than one is asked for, each must be a genuinely DISTINCT interpretation (different composition, setting variation, lighting, or creative angle — all honoring the same instruction and subject), not a paraphrase of the same picture.
...
- Write prompt(s) that DESCRIBE the core subject concretely enough to be recognizable in words (shape, color, material, defining details) AND apply the requested changes. Be specific and visual.
...
{{
  ...
  "enhanced_prompts": ["exactly {num_options} vivid, self-contained text-to-image prompt(s) (each <= {max_chars} chars) that a model will actually receive — core subject described + requested changes applied; each a distinct interpretation"],
...
- enhanced_prompts is a JSON array of exactly {num_options} plain STRING entries — no objects, no key:value pairs, and no extra keys inside the array. Each entry is ONE model-ready caption, not a list of instructions — write it as a description of the final image.
- Keep each prompt under {max_chars} characters.
- Output ONLY the JSON object — strict JSON: no // comments, no trailing commas, no markdown fences, and escape any double quotes inside strings.""",
    },
    "image_refine_single": {
