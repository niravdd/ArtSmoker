...
6. **NEGATIVE: line** — include ONLY if the model supports negative prompts (check MODEL instructions above). If the model says NO negative prompts, skip this entirely and focus all effort on the positive caption.
   ...
   - Quality (all): blurry, low quality, text, watermark, signature, jpeg artifacts
   - Only if the subject is clearly meant to be a realistic/standard living being AND the user did NOT ask for a stylized or non-standard body: bad anatomy, deformed. DO NOT add anatomy negatives (extra limbs/fingers/heads) when the subject is a creature, robot, monster, or when the user deliberately described a non-standard form — that would fight their intent.
