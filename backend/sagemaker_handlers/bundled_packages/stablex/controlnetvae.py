# Vendored from Stable-X/yoso-normal-v1-8-1 (and yoso-delight-v0-4-base),
# file controlnet/controlnetvae.py. Apache-2.0 (HuggingFace Team copyright,
# header preserved below). This is the ControlNetVAEModel subclass the YOSO
# one-step pipelines need: it overrides forward() to use `sample` (the
# VAE-encoded image latent) as the conditioning and makes `controlnet_cond`
# optional — stock diffusers ControlNetModel REQUIRES controlnet_cond, which is
# why loading these weights into stock ControlNetModel raised
# 'forward() missing controlnet_cond'. We drive it ourselves in
# inference.py::_estimate_normal / _delight_image. Commercial-clean.
