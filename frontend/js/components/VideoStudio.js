    ...
    window.VideoStudio = {
        ...
        render() {
            return `
                <div id="video-studio-view" class="view-enter">
                    <div class="mb-6">
                        <h1 class="text-2xl font-bold">${t('artsmoker.video_studio.title')}</h1>
                        <p class="text-brand-text-muted text-sm mt-1">${t('artsmoker.video_studio.subtitle')}</p>
                    </div>
                        ...
                        <aside class="lg:w-72 xl:w-80 flex-shrink-0 space-y-5">
                            <div class="card-static p-5 space-y-5">
                                <h2 class="text-lg font-semibold flex items-center gap-2">
                                    ...
                                    ${t('artsmoker.video_studio.settings_heading')}
                                </h2>

                                <!-- Model -->
                                <div>
                                    <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.model')}</label>
                                    <div class="select-expand-wrap">
                                        <select id="vs-model" class="input">
                                            <option value="">${t('artsmoker.video_studio.model_loading')}</option>
                                        </select>
                                    </div>
                                ...
                                <div id="vs-task-type-group" class="hidden">
                                    <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.generation_mode')}</label>
                                    <select id="vs-task-type" class="input">
                                        <option value="TEXT_VIDEO">${t('artsmoker.video_studio.task_single')}</option>
                                        <option value="MULTI_SHOT_AUTOMATED">${t('artsmoker.video_studio.task_multi_auto')}</option>
                                        <option value="MULTI_SHOT_MANUAL">${t('artsmoker.video_studio.task_multi_manual')}</option>
                                    </select>
                                </div>

                                <!-- Duration -->
                                <div>
                                    <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.duration')}</label>
                                    <select id="vs-duration" class="input">
                                        <option value="6">6 ${t('artsmoker.video_studio.seconds')}</option>
                                    </select>
                                </div>

                                <!-- Aspect Ratio (Luma only) -->
                                <div id="vs-aspect-group" class="hidden">
                                    <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.aspect_ratio')}</label>
                                ...
                                <div id="vs-resolution-group" class="hidden">
                                    <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.resolution')}</label>
                                ...
                                <details class="group">
                                    <summary class="text-xs font-medium text-brand-text-muted cursor-pointer hover:text-brand-text transition-colors select-none">
                                        <span class="group-open:hidden">\u25B8 ${t('artsmoker.video_studio.advanced_collapsed')}</span>
                                        <span class="hidden group-open:inline">\u25BE ${t('artsmoker.video_studio.advanced_expanded')}</span>
                                    </summary>
                                    <div class="mt-2 space-y-3 p-2.5 rounded-lg bg-brand-bg/40 border border-brand-border/50">
                                        <div>
                                            <label class="block text-[10px] text-brand-text-muted uppercase tracking-wider mb-1">${t('artsmoker.common.region')}</label>
                                            <select id="vs-region" class="input text-xs">
                                                <option value="">${t('artsmoker.video_studio.auto')}</option>
                                            </select>
                                        </div>
                                        <div>
                                            <label class="block text-[10px] text-brand-text-muted uppercase tracking-wider mb-1">${t('artsmoker.video_studio.seed')}</label>
                                            <input type="number" id="vs-seed" class="input text-xs w-full" placeholder="${t('artsmoker.video_studio.seed_placeholder')}" min="0">
                                        </div>
                                    </div>
                                </details>
                                ...
                                <details id="vs-image-input-group" class="hidden">
                                    <summary class="text-xs font-medium cursor-pointer text-brand-accent hover:text-brand-accent-hover select-none">
                                        \u25B8 ${t('artsmoker.video_studio.source_image')}
                                    </summary>
                                    <div class="mt-2 p-2.5 border border-brand-border/50 rounded-lg bg-brand-bg/40 space-y-2">
                                        ...
                                        <p class="text-[10px] text-brand-text-muted">${t('artsmoker.video_studio.source_image_hint')}</p>
                                        <div id="vs-source-preview" class="hidden">
                                            <img id="vs-source-img" class="max-h-24 rounded border border-brand-border" alt="${t('artsmoker.common.source')}">
                                            <button id="vs-clear-source" class="btn btn-secondary btn-sm mt-1 text-[10px]">${t('artsmoker.video_studio.source_image_clear')}</button>
                                        </div>
                                    </div>
                                </details>
                            </div>
                            ...
                            <!-- Settings buttons -->
                            <button id="vs-model-settings-btn" class="w-full text-left p-3 rounded-lg bg-emerald-700 hover:bg-emerald-600 border border-emerald-500/30 transition-colors flex items-center gap-2 text-xs text-white font-medium">
                                ...
                                ${t('artsmoker.video_studio.model_settings')}
                            </button>
                            <button id="vs-settings-btn" class="w-full text-left p-3 rounded-lg bg-brand-bg/30 border border-brand-border/50 hover:border-brand-accent/30 hover:bg-brand-bg/50 transition-colors flex items-center gap-2 text-xs text-brand-text-muted">
                                ...
                                ${t('artsmoker.video_studio.video_settings')}
                            </button>
                        ...
                        <div class="flex-1 min-w-0 space-y-5">
                            ...
                            <div class="card-static p-5 space-y-4">
                                <h2 class="text-lg font-semibold flex items-center gap-2">
                                    ...
                                    ${t('artsmoker.common.prompt')}
                                </h2>
                                <textarea id="vs-prompt" rows="4" class="input w-full"
                                    placeholder="${t('artsmoker.video_studio.prompt_placeholder')}"></textarea>
                                <!-- Translation preview -->
                                <div id="vs-translation-preview" class="hidden">
                                    <div class="flex items-center gap-1 my-1">
                                        ...
                                        <div class="flex gap-0.5 ml-auto">
                                            <button type="button" id="vs-tab-original" class="text-[10px] px-2 py-0.5 rounded bg-brand-accent text-white font-medium">${t('artsmoker.common.prompt') || 'Original'}</button>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <label class="flex items-center gap-1.5 text-xs text-brand-text-muted cursor-pointer">
                                        ...
                                        ${t('artsmoker.video_studio.enhance_prompt')}
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <button id="vs-generate-btn" class="btn btn-primary btn-lg text-base" disabled>
                                    ...
                                    ${t('artsmoker.video_studio.generate')}
                                </button>
                            ...
                            <div id="vs-prompt-info" class="hidden card-static p-4 space-y-3">
                                <div>
                                    <p class="text-[10px] text-brand-text-muted uppercase tracking-wider font-semibold mb-1">${t('artsmoker.video_studio.original_prompt')}</p>
                                    <p id="vs-original-prompt" class="text-sm text-brand-text/80 leading-relaxed"></p>
                                </div>
                                <div id="vs-enhanced-prompt-section" class="hidden">
                                    <p class="text-[10px] text-brand-accent uppercase tracking-wider font-semibold mb-1">${t('artsmoker.video_studio.enhanced_prompt')}</p>
                                    <p id="vs-enhanced-prompt" class="text-sm text-brand-text/60 leading-relaxed"></p>
                                </div>
                                <div id="vs-negative-section" class="hidden">
                                    <p class="text-[10px] text-amber-400/80 uppercase tracking-wider font-semibold mb-1">${t('artsmoker.video_studio.negative_concepts_note')}</p>
                            ...
                            <div id="vs-jobs-section" class="card-static p-4 hidden">
                                <h3 class="text-sm font-semibold mb-3 flex items-center gap-2">
                                    ...
                                    ${t('artsmoker.video_studio.active_jobs')}
                                </h3>
                            ...
                            <div id="vs-completed" class="hidden space-y-4">
                                <h2 class="text-lg font-semibold">${t('artsmoker.video_studio.recent_videos')}</h2>
                ...
                <div id="vs-settings-dialog" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div class="card-static p-6 w-full max-w-lg space-y-4">
                        <div class="flex items-center justify-between">
                            <h3 class="text-lg font-semibold">${t('artsmoker.video_studio.settings_title')}</h3>
                            <button id="vs-settings-close" class="text-brand-text-muted hover:text-brand-text text-xl">&times;</button>
                        </div>
                        <div class="space-y-4">
                            <!-- S3 Bucket Selection -->
                            <div>
                                <label class="block text-sm font-medium mb-1.5">${t('artsmoker.video_studio.s3_bucket')}</label>
                                <div class="flex gap-2">
                                    <input type="text" id="vs-s3-bucket" class="input flex-1" placeholder="${t('artsmoker.video_studio.s3_bucket_placeholder')}">
                                    <button id="vs-browse-s3" class="btn btn-secondary btn-sm whitespace-nowrap">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>
                                        </svg>
                                        ${t('artsmoker.video_studio.s3_browse')}
                                    </button>
                                </div>
                                <p class="text-[10px] text-brand-text-muted mt-1">${t('artsmoker.video_studio.s3_bucket_hint')}</p>
                            </div>
                            ...
                            <div id="vs-bucket-list" class="hidden border border-brand-border rounded-lg overflow-hidden">
                                <div class="px-3 py-2 bg-brand-bg/60 border-b border-brand-border flex items-center justify-between">
                                    <span class="text-xs font-medium text-brand-text-muted">${t('artsmoker.video_studio.your_s3_buckets')}</span>
                                    <button id="vs-create-bucket-btn" class="text-[10px] text-brand-accent hover:text-brand-accent-hover font-medium">${t('artsmoker.video_studio.create_new')}</button>
                                </div>
                                <div id="vs-bucket-items" class="max-h-40 overflow-auto">
                                    <div class="text-xs text-brand-text-muted p-3 text-center">${t('artsmoker.common.loading')}</div>
                                </div>
                            </div>
                            ...
                            <div id="vs-create-bucket-form" class="hidden p-3 border border-brand-accent/30 rounded-lg bg-brand-accent/5 space-y-2">
                                <label class="block text-xs font-medium">${t('artsmoker.video_studio.new_bucket_name')}</label>
                                <div class="flex gap-2">
                                ...
                                <p class="text-[10px] text-brand-text-muted">${t('artsmoker.video_studio.bucket_name_hint')}</p>
                                <div class="flex gap-2">
                                    <button id="vs-create-bucket-go" class="btn btn-primary btn-sm text-xs">${t('artsmoker.video_studio.s3_create')}</button>
                                    <button id="vs-create-bucket-cancel" class="btn btn-secondary btn-sm text-xs">${t('artsmoker.common.cancel')}</button>
                                </div>
                            </div>

                            <!-- S3 Prefix -->
                            <div>
                                <label class="block text-sm font-medium mb-1">${t('artsmoker.video_studio.s3_prefix')}</label>
                                <input type="text" id="vs-s3-prefix" class="input w-full" placeholder="artsmoker/video/" value="artsmoker/video/">
                            </div>

                            <!-- Storage mode -->
                            <div>
                                <label class="block text-sm font-medium mb-1">${t('artsmoker.video_studio.s3_storage_mode')}</label>
                                <select id="vs-store-mode" class="input w-full">
                                    <option value="local">${t('artsmoker.video_studio.s3_local')}</option>
                                    <option value="s3">${t('artsmoker.video_studio.s3_only')}</option>
                                </select>
                                <p class="text-[10px] text-brand-text-muted mt-1">
                                    <strong>${t('artsmoker.video_studio.s3_local_desc_local')}</strong><br>
                                    <strong>${t('artsmoker.video_studio.s3_local_desc_s3')}</strong>
                                </p>
                            </div>

                            <div id="vs-s3-status" class="hidden text-xs p-2 rounded"></div>
                            <div class="flex gap-2 justify-end">
                                <button id="vs-settings-close-bottom" class="btn btn-secondary btn-sm hidden">${t('artsmoker.common.close')}</button>
                                <button id="vs-settings-test" class="btn btn-primary btn-sm">${t('artsmoker.video_studio.s3_test_save')}</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        _attachEvents() {
            ...
            document.getElementById('vs-s3-bucket')?.addEventListener('input', () => {
                ...
                if (testBtn) { testBtn.disabled = false; testBtn.textContent = t('artsmoker.video_studio.s3_test_save'); }
        ...
        async _loadModels() {
            ...
            if (this._models.length === 0) {
                // nosemgrep
                sel.innerHTML = html`<option value="">${t('artsmoker.video_studio.model_not_found')}</option>`;
                return;
            }
        ...
        _updateRegionForModel(model) {
            ...
            if (regions.length <= 1) {
                ...
                auto.textContent = price
                    ? `${t('artsmoker.video_studio.auto')} \u2014 ${defaultRegion} ($${price}/sec)`
                    : `${t('artsmoker.video_studio.auto')} \u2014 ${defaultRegion}`;
        ...
        _updateDurationOptions(model, isNovaReel) {
            ...
            if (isNovaReel) {
                ...
                if (taskType === 'TEXT_VIDEO') {
                    // nosemgrep
                    durSel.innerHTML = html`<option value="6">6 ${t('artsmoker.video_studio.seconds')}</option>`;
                } else {
                    ...
                    for (let d = min; d <= max; d += step) {
                        const opt = document.createElement('option');
                        opt.value = d;
                        opt.textContent = d >= 60 ? `${Math.floor(d/60)}m ${d%60 ? d%60 + 's' : ''}` : `${d} ${t('artsmoker.video_studio.seconds')}`;
        ...
        _updateCostEstimate() {
            ...
            el.textContent = t('artsmoker.video_studio.est_cost_detail')
                .replace('{{cost}}', cost)
        ...
        async _generate() {
            ...
            if (!this._videoSettings?.s3_bucket) {
                window.showToast?.(t('artsmoker.video_studio.s3_not_configured'), 'warning');
                this._showSettings();
                return;
            }
            ...
            if (!prompt) {
                window.showToast?.(t('artsmoker.video_studio.enter_prompt'), 'warning');
                return;
            }

            const modelKey = document.getElementById('vs-model')?.value;
            if (!modelKey) {
                window.showToast?.(t('artsmoker.video_studio.select_model'), 'warning');
                return;
            }
            ...
            if (btn) { btn.disabled = true; btn.textContent = t('artsmoker.video_studio.starting'); }

            try {
                const job = await API.video.generate(payload);
                window.showToast?.(`${t('artsmoker.video_studio.started')}: ${job.model_label}`, 'success');
                ...
                if (isS3Issue) {
                    window.showToast?.(t('artsmoker.video_studio.s3_issue'), 'warning');
                    this._showSettings();
                    ...
                    window.showToast?.(t('artsmoker.video_studio.generation_failed').replace('{{msg}}', msg), 'error');
                }
            } finally {
                if (btn) {
                    btn.disabled = false;
                    // nosemgrep
                    btn.innerHTML = html`<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> ${t('artsmoker.video_studio.generate')}`;
                }
            }
        },
            ...
            this._pollTimers[jobId] = setInterval(async () => {
                try {
                    ...
                    if (status.status === 'Completed') {
                        ...
                        window.showToast?.(t('artsmoker.video_studio.complete'), 'success');
                        this._loadRecentJobs();
                        // Refresh gallery if visible
                        if (window.Gallery?.refresh) window.Gallery.refresh();
                    } else if (status.status === 'Failed') {
                        this._stopPolling(jobId);
                        window.showToast?.(t('artsmoker.video_studio.video_failed').replace('{{msg}}', status.failure_message || t('artsmoker.common.unknown')), 'error');
                        this._renderJobsList(this._activeJobs.filter(j => j.status === 'InProgress'));
                    }
                    this._renderJobsList(this._activeJobs.filter(j => j.status === 'InProgress'));
                } catch (_) {}
                ...
                return html`
                    <div class="p-3 rounded-lg bg-brand-bg border border-brand-border space-y-1.5">
                        <div class="flex items-center gap-2">
                            ...
                            ${hasEnhanced ? html`<span class="text-[9px] text-brand-accent">${t('artsmoker.video_studio.ai_enhanced')}</span>` : ''}
                        </div>
                        <p class="text-xs text-brand-text-muted line-clamp-2">${j.original_prompt || j.prompt || ''}</p>
                        ${hasEnhanced ? html`<details class="group"><summary class="text-[10px] text-brand-accent cursor-pointer">${t('artsmoker.video_studio.show_enhanced')}</summary><p class="text-[10px] text-brand-text/50 mt-1 leading-relaxed">${j.enhanced_prompt}</p></details>` : ''}
        ...
        _openVideoPlayer(videoId) {
            ...
            overlay.innerHTML = html`
                <div class="relative max-w-4xl w-full mx-4">
                    ...
                    <video controls autoplay class="w-full rounded-lg shadow-2xl" src="${mp4Url}">
                        ${t('artsmoker.video_studio.browser_no_video')}
                    </video>
                    ...
                    <div class="vs-player-meta mt-3 rounded-lg bg-brand-surface/90 p-4 space-y-2 hidden">
                        <div class="vs-meta-original">
                            <p class="text-[10px] text-brand-text-muted uppercase tracking-wider font-semibold mb-0.5">${t('artsmoker.video_studio.original_prompt')}</p>
                            <p class="vs-meta-original-text text-sm text-brand-text/80 leading-relaxed"></p>
                        </div>
                        <div class="vs-meta-enhanced hidden">
                            <p class="text-[10px] text-brand-accent uppercase tracking-wider font-semibold mb-0.5">${t('artsmoker.video_studio.enhanced_prompt')}</p>
                            <p class="vs-meta-enhanced-text text-sm text-brand-text/60 leading-relaxed"></p>
                        </div>
                        <div class="vs-meta-negative hidden">
                            <p class="text-[10px] text-amber-400/80 uppercase tracking-wider font-semibold mb-0.5">${t('artsmoker.video_studio.negative_concepts')}</p>
            ...
            API.video.metadata(videoId).then(meta => {
                ...
                if (modelEl) modelEl.textContent = `${t('artsmoker.video_studio.meta_model')}: ${meta.model_label || meta.model_key || ''}`;
                const durEl = metaEl.querySelector('.vs-meta-duration');
                if (durEl && meta.duration_seconds) durEl.textContent = `${t('artsmoker.video_studio.meta_duration')}: ${Math.round(meta.duration_seconds)}s`;
                const regEl = metaEl.querySelector('.vs-meta-region');
                if (regEl && meta.region) regEl.textContent = `${t('artsmoker.video_studio.meta_region')}: ${meta.region}`;
            }).catch(() => {});
            ...
            overlay.querySelector('.vs-revise-btn')?.addEventListener('click', async () => {
                ...
                try {
                    ...
                    this._updateCharCount();
                    window.showToast?.(t('artsmoker.video_studio.prompt_loaded'), 'info');
                } catch (_) {}
            });

            // Delete handler
            overlay.querySelector('.vs-delete-btn')?.addEventListener('click', async () => {
                if (!await window.showConfirm(t('artsmoker.video_studio.delete_confirm'), { title: t('artsmoker.video_studio.delete_title'), detail: t('artsmoker.video_studio.delete_detail'), confirmLabel: t('artsmoker.common.delete'), danger: true })) return;
                try {
                    await API.video.delete(videoId);
                    overlay.remove();
                    window.showToast?.(t('artsmoker.video_studio.deleted'), 'success');
                    this._loadRecentJobs();
                } catch (_) {}
            });
        },
        ...
        _updateS3Banner() {
            ...
            if (!vs.s3_bucket) {
                ...
                content.innerHTML = html`
                    ...
                    <div>
                        <p class="text-sm text-amber-200 font-medium">${t('artsmoker.video_studio.s3_required')}</p>
                        <p class="text-xs text-amber-300/70 mt-1">${t('artsmoker.video_studio.s3_required_desc')}</p>
                    </div>
                `;
                if (genBtn) genBtn.disabled = true;
            } else {
                const storeMode = vs.store_local !== false ? t('artsmoker.video_studio.s3_local_plus') : t('artsmoker.video_studio.s3_only_label');
                banner.className = banner.className.replace(/bg-\S+/g, '').replace(/border-\S+/g, '') +
                ...
                content.innerHTML = html`
                    ...
                    <div class="flex-1">
                        <p class="text-sm text-emerald-200 font-medium">${t('artsmoker.video_studio.s3_configured')}: <span class="font-mono">${vs.s3_bucket}</span></p>
                        <p class="text-xs text-emerald-300/60 mt-0.5">${t('artsmoker.video_studio.s3_prefix_label')}: ${vs.s3_prefix || 'artsmoker/video/'} · ${t('artsmoker.video_studio.s3_storage_label')}: ${storeMode}</p>
                    </div>
                `;
            }
        },
        ...
        async _testAndSaveSettings() {
            ...
            if (!bucket) {
                if (statusEl) {
                    statusEl.classList.remove('hidden');
                    statusEl.className = 'text-xs p-2 rounded bg-red-950/50 text-red-300';
                    statusEl.textContent = t('artsmoker.video_studio.bucket_required');
                }
                return;
            }
            ...
            if (statusEl) {
                ...
                statusEl.textContent = t('artsmoker.video_studio.s3_testing');
            }
            ...
            try {
                ...
                if (statusEl) {
                    ...
                    statusEl.textContent = t('artsmoker.video_studio.s3_validated_msg').replace('{{name}}', bucket);
                }
                ...
                if (testBtn) { testBtn.disabled = true; testBtn.textContent = t('artsmoker.video_studio.saved'); }
                if (closeBtn) { closeBtn.classList.remove('hidden'); }
                closeBtn?.addEventListener('click', () => this._hideSettings(), { once: true });

                window.showToast?.(t('artsmoker.video_studio.settings_saved'), 'success');
            } catch (err) {
                ...
                if (isNotFound) {
                    ...
                    if (statusEl) {
                        ...
                        statusEl.innerHTML = html`
                            <p>${t('artsmoker.video_studio.bucket_not_exist').replace('{{name}}', bucket)}</p>
                            <div class="flex items-center gap-2">
                                <button id="vs-create-inline-btn" class="btn btn-primary btn-sm text-xs">${t('artsmoker.video_studio.create_it_now')}</button>
                        ...
                        document.getElementById('vs-create-inline-btn')?.addEventListener('click', async () => {
                            ...
                            // nosemgrep
                            statusEl.innerHTML = html`<p>${t('artsmoker.video_studio.creating_bucket_status')}</p>`;
                            try {
                                await API.browse.createS3Bucket(bucket, region);
                                window.showToast?.(t('artsmoker.video_studio.bucket_created').replace('{{name}}', bucket).replace('{{region}}', region), 'success');
                                // Retry the save now that the bucket exists
                                await this._testAndSaveSettings();
                            } catch (createErr) {
                                statusEl.className = 'text-xs p-2 rounded bg-red-950/50 text-red-300';
                                statusEl.textContent = t('artsmoker.video_studio.create_failed').replace('{{msg}}', createErr.message || '');
                            }
                        });
                    }
                    return;
        ...
        async _loadBucketList() {
            ...
            document.getElementById('vs-create-bucket-form')?.classList.add('hidden');
            // nosemgrep
            itemsEl.innerHTML = html`<div class="text-xs text-brand-text-muted p-3 text-center">${t('artsmoker.video_studio.loading_buckets')}</div>`;
            ...
            try {
                ...
                if (buckets.length === 0) {
                    // nosemgrep
                    itemsEl.innerHTML = html`<div class="text-xs text-brand-text-muted p-3 text-center">${t('artsmoker.video_studio.no_buckets')}</div>`;
                    return;
                }
                ...
                // nosemgrep
                itemsEl.innerHTML = html`<div class="text-xs text-red-400 p-3 text-center">${err.message || t('artsmoker.common.error')}</div>`;
            }
        },
        ...
        async _createBucket() {
            ...
            if (!name) {
                window.showToast?.(t('artsmoker.video_studio.bucket_name_required'), 'warning');
                return;
            }

            const goBtn = document.getElementById('vs-create-bucket-go');
            if (goBtn) { goBtn.disabled = true; goBtn.textContent = t('artsmoker.video_studio.s3_creating'); }

            try {
                const result = await API.browse.createS3Bucket(name, region);
                window.showToast?.(
                    result.created
                        ? t('artsmoker.video_studio.bucket_created').replace('{{name}}', name).replace('{{region}}', region)
                        : t('artsmoker.video_studio.bucket_exists').replace('{{name}}', name),
                    'success'
                );
                ...
                window.showToast?.(t('artsmoker.video_studio.create_failed').replace('{{msg}}', err.message || ''), 'error');
            } finally {
                if (goBtn) { goBtn.disabled = false; goBtn.textContent = t('artsmoker.video_studio.s3_create'); }
            }
        },
    };
