...
(function () {
    ...
    window.Gallery = {
            ...
            return html`
                <div id="gallery-view" class="space-y-6 view-enter">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 class="text-2xl font-bold">${t('artsmoker.ui.gallery.title')}</h1>
                            <p class="text-sm text-brand-text-muted mt-1">${t('artsmoker.ui.gallery.subtitle')}</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <button id="gal-import-btn" class="btn btn-sm bg-cyan-600 hover:bg-cyan-500 text-white">
                                ...
                                ${t('artsmoker.ui.gallery.import_image')}
                            </button>
                            <a href="#image-studio" class="btn btn-primary btn-sm">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
                                ${t('artsmoker.ui.gallery.generate_new')}
                            </a>
                        </div>
                    </div>
                    ...
                    <div id="gal-selection-bar" class="hidden card-static p-3 bg-red-950/30 border-red-500/30 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <button id="gal-select-all" class="btn btn-secondary btn-sm text-xs">${t('artsmoker.ui.gallery.select_all')}</button>
                            <button id="gal-deselect-all" class="btn btn-secondary btn-sm text-xs">${t('artsmoker.ui.gallery.deselect_all')}</button>
                            <span id="gal-selected-count" class="text-sm text-red-300 font-medium"></span>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-[10px] text-red-400/70">${t('artsmoker.ui.gallery.delete_warning')}</span>
                            <button id="gal-delete-btn" class="btn btn-sm bg-red-600 hover:bg-red-500 text-white">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                </svg>
                                ${t('artsmoker.ui.gallery.delete_selected')}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="card-static p-4 space-y-3">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="gal-search" class="input w-full pl-10" placeholder="${t('artsmoker.ui.gallery.search_placeholder')}">
                        </div>
                        <!-- Filters -->
                        <div class="flex flex-wrap items-center gap-3">
                            <div class="flex-1 min-w-[120px]">
                                <label class="block text-xs text-brand-text-muted mb-1">${t('artsmoker.ui.gallery.filter_media')}</label>
                                <select id="gal-filter-media" class="input">
                                    <option value="">${t('artsmoker.ui.gallery.filter_all_media')}</option>
                                    <option value="image">${t('artsmoker.ui.gallery.filter_images')}</option>
                                    <option value="3d">${t('artsmoker.ui.gallery.filter_3d')}</option>
                                    <option value="video">${t('artsmoker.ui.gallery.filter_videos')}</option>
                                </select>
                            </div>
                            <div class="flex-1 min-w-[160px]">
                                <label class="block text-xs text-brand-text-muted mb-1">${t('artsmoker.ui.gallery.filter_style')}</label>
                                <select id="gal-filter-style" class="input">
                                    <option value="">${t('artsmoker.ui.gallery.filter_all_styles')}</option>
                                </select>
                            </div>
                            <div class="flex-1 min-w-[160px]">
                                <label class="block text-xs text-brand-text-muted mb-1">${t('artsmoker.ui.gallery.filter_type')}</label>
                                <select id="gal-filter-type" class="input">
                                    <option value="">${t('artsmoker.ui.gallery.filter_all_types')}</option>
                                    <option value="game_asset">${t('artsmoker.ui.gallery.filter_game_asset')}</option>
                                    <option value="marketing_banner">${t('artsmoker.ui.gallery.filter_marketing_banner')}</option>
                                    <option value="icon">${t('artsmoker.ui.gallery.filter_icon')}</option>
                                    <option value="character">${t('artsmoker.ui.gallery.filter_character')}</option>
                                    <option value="environment">${t('artsmoker.ui.gallery.filter_environment')}</option>
                                    <option value="video">${t('artsmoker.ui.gallery.filter_video')}</option>
                                </select>
                            </div>
                            <div class="flex-1 min-w-[160px]">
                                <label class="block text-xs text-brand-text-muted mb-1">${t('artsmoker.ui.gallery.sort')}</label>
                                <select id="gal-sort" class="input">
                                    <option value="newest">${t('artsmoker.ui.gallery.sort_newest')}</option>
                                    <option value="oldest">${t('artsmoker.ui.gallery.sort_oldest')}</option>
                                </select>
                            </div>
                    ...
                    <div id="gal-load-more" class="hidden text-center py-4">
                        <button id="btn-load-more" class="btn btn-secondary btn-sm">${t('artsmoker.ui.gallery.load_more')}</button>
                    </div>
        ...
        _openImportModal() {
            ...
            const TYPES = [
                ['character', t('artsmoker.ui.gallery.filter_character')],
                ['game_asset', t('artsmoker.ui.gallery.filter_game_asset')],
                ['environment', t('artsmoker.ui.gallery.filter_environment')],
                ['icon', t('artsmoker.ui.gallery.filter_icon')],
                ['marketing_banner', t('artsmoker.ui.gallery.filter_marketing_banner')],
                ['photorealistic', t('artsmoker.ui.gallery.filter_photorealistic')],
            ];
            ...
            backdrop.innerHTML = html`
                <div class="bg-brand-surface rounded-xl border border-brand-border shadow-2xl max-w-lg w-full p-5 space-y-4 max-h-[92vh] overflow-y-auto">
                    <div>
                        <h3 class="text-sm font-semibold text-brand-text">${t('artsmoker.ui.gallery.import_title')}</h3>
                        <p class="text-[11px] text-brand-text-dim mt-1">${t('artsmoker.ui.gallery.import_subtitle')}</p>
                    </div>
                    ...
                    <div>
                        <label class="text-[10px] text-brand-text-muted uppercase tracking-wider mb-1 block">${t('artsmoker.ui.gallery.import_asset_type')}</label>
                        <select id="gi-type" class="input text-sm w-full">
                            <option value="">${t('artsmoker.ui.gallery.import_asset_type_ph')}</option>
                            ${TYPES.map(([v, l]) => html`<option value="${v}">${l}</option>`)}
                        </select>
                        <p class="text-[9px] text-brand-text-dim mt-1">${t('artsmoker.ui.gallery.import_asset_type_hint')}</p>
                    </div>
                    <div>
                        <label class="text-[10px] text-brand-text-muted uppercase tracking-wider mb-1 block">${t('artsmoker.ui.gallery.import_title_label')}</label>
                        <input id="gi-title" type="text" class="input text-sm w-full" placeholder="${t('artsmoker.ui.gallery.import_title_ph')}" />
                    </div>
                    <div class="flex flex-wrap gap-4">
                        <label class="flex items-center gap-2 cursor-pointer text-[11px] text-brand-text">
                            <input id="gi-ip-owned" type="checkbox" class="accent-brand-accent" /> ${t('artsmoker.ui.gallery.import_ip_owned')}
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer text-[11px] text-brand-text">
                            <input id="gi-ip-licensed" type="checkbox" class="accent-brand-accent" /> ${t('artsmoker.ui.gallery.import_ip_licensed')}
                        </label>
                    </div>
                    <div class="flex items-center gap-2">
                        <button id="gi-submit" class="btn btn-primary btn-sm flex-1" disabled>${t('artsmoker.ui.gallery.import_submit')}</button>
                        <button id="gi-cancel" class="btn btn-secondary btn-sm">${t('artsmoker.ui.prompt_designer.cancel')}</button>
                    </div>
                </div>`;
            ...
            const setFile = (f) => {
                if (!f || !f.type.startsWith('image/')) { window.showToast?.(t('artsmoker.ui.gallery.import_pick_file'), 'warning'); return; }
                chosenFile = f;
            ...
            submitBtn.addEventListener('click', async () => {
                if (!chosenFile) { window.showToast?.(t('artsmoker.ui.gallery.import_pick_file'), 'warning'); return; }
                if (!typeSel.value) { window.showToast?.(t('artsmoker.ui.gallery.import_pick_type'), 'warning'); return; }
                submitBtn.disabled = true;
                // nosemgrep
                submitBtn.innerHTML = html`<span class="spinner-sm"></span> ${t('artsmoker.ui.gallery.import_importing')}`;
                try {
                    ...
                    window.showToast?.(t('artsmoker.ui.gallery.import_success'), 'success');
                    close();
                    await this.refresh();
                    ...
                    window.showToast?.(t('artsmoker.ui.gallery.import_failed') + ': ' + (err.message || ''), 'error');
                    submitBtn.disabled = false;
                    submitBtn.textContent = t('artsmoker.ui.gallery.import_submit');
                }
            });
        ...
        async _loadItems(reset = true) {
            ...
            try {
                ...
                if (grid && this._items.length === 0) {
                    // nosemgrep
                    grid.innerHTML = html`<div class="col-span-full text-center py-8 text-red-400">${t('artsmoker.ui.gallery.load_error')}</div>`;
                }
            } finally {
                this._loading = false;
            }
        ...
        _updateSelectionUI() {
            ...
            if (count) count.textContent = t('artsmoker.ui.gallery.selected', { count: n, plural: n !== 1 ? 's' : '' });
        ...
        async _handleDelete() {
            ...
            if (!await window.showConfirm(t('artsmoker.ui.gallery.delete_confirm', { count: ids.length, plural: ids.length !== 1 ? 's' : '' }), { title: t('artsmoker.ui.gallery.delete_title'), detail: t('artsmoker.ui.gallery.delete_detail'), confirmLabel: t('artsmoker.ui.gallery.confirm_label'), danger: true })) return;

            window.showLoading?.(t('artsmoker.ui.gallery.deleting', { count: ids.length, plural: ids.length !== 1 ? 's' : '' }));
            try {
                ...
                window.hideLoading?.();
                this._selected.clear();
                window.showToast?.(t('artsmoker.ui.gallery.deleted', { count: deletedCount, plural: deletedCount !== 1 ? 's' : '' }), 'success');
        ...
        _updateLoadMore() {
            ...
            if (countEl) {
                ...
                const countText = t('artsmoker.ui.gallery.asset_count', { count: this._items.length, plural: this._items.length !== 1 ? 's' : '' });
                const searchNote = this._filteredItems !== null ? t('artsmoker.ui.gallery.matching', { count: displayItems.length }) : '';
                const moreNote = this._hasMore ? t('artsmoker.ui.gallery.more_available') : '';
            ...
            if (displayItems.length === 0 && this._items.length > 0) {
                ...
                grid.innerHTML = html`
                    <div class="col-span-full text-center py-12 text-brand-text-muted">
                        <p class="text-sm">${t('artsmoker.ui.gallery.no_results')}</p>
                    </div>
                `;
                return;
            }
            ...
            if (this._items.length === 0) {
                ...
                grid.innerHTML = html`
                    <div class="col-span-full empty-state py-16">
                        ...
                        <h3 class="text-lg font-semibold text-brand-text mb-1">${t('artsmoker.ui.gallery.no_assets')}</h3>
                        <p class="text-brand-text-muted text-sm mb-4">${t('artsmoker.ui.gallery.no_assets_desc')}</p>
                        <a href="#image-studio" class="btn btn-primary btn-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                            </svg>
                            ${t('artsmoker.ui.gallery.go_to_studio')}
                        </a>
                    </div>
                `;
                return;
                    ...
                    <div class="img-hover-zoom ${isVideo || isPlaceholder ? (isVideo ? 'aspect-video' : 'aspect-[4/3]') : ''} bg-brand-bg flex items-center justify-center overflow-hidden relative">
                        ${item.async_status === 'pending' || item.async_status === 'generating'
                            ...
                            ? html`<div class="w-full h-full flex flex-col items-center justify-center text-amber-400/70 gap-2 px-2 text-center">
                                ...
                                <span class="text-[10px]">${t('artsmoker.ui.gallery.status_censored')}</span>
                               </div>`
                            ...
                            ? html`<div class="w-full h-full flex flex-col items-center justify-center text-red-400/60 gap-2">
                                ...
                                <span class="text-[10px]">${t('artsmoker.ui.gallery.status_failed')}</span>
                               </div>`
                            : html`<img src="${thumbUrl}" alt="${isVideo ? t('artsmoker.ui.gallery.alt_video_thumb') : t('artsmoker.ui.gallery.alt_asset')}"
                        ...
                        ${isVideo ? html`<span class="absolute top-2 right-2 bg-brand-accent/80 text-white text-[9px] px-1.5 py-0.5 rounded font-medium">${t('artsmoker.ui.gallery.video_badge')}</span>` : ''}
                        ${item.has_3d ? html`<span class="absolute bottom-2 right-2 z-10 flex items-center gap-1 bg-violet-600/85 text-white text-[9px] px-1.5 py-0.5 rounded font-semibold shadow" title="${t('artsmoker.ui.gallery.has_3d_tooltip')}">
                            ...
                            ${t('artsmoker.ui.gallery.has_3d_badge')}
                        </span>` : ''}
                    </div>
                    <div class="p-4 space-y-2">
                        <p class="text-sm text-brand-text line-clamp-2 group-hover:text-brand-accent transition-colors">${truncPrompt || html`<em class="text-brand-text-muted">${t('artsmoker.ui.gallery.no_prompt')}</em>`}</p>
                        ...
                        <div class="flex items-center flex-wrap gap-2 text-xs text-brand-text-muted">
                            ${item.image_model === 'imported'
                                ? html`<span class="px-1.5 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">${t('artsmoker.ui.gallery.imported_badge')}</span>`
