    ...
    window.StyleLibrary = {
            ...
            return html`
                <div id="style-library" class="space-y-6 view-enter">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div>
                            <h1 class="text-2xl font-bold">${t('artsmoker.style_library.title')}</h1>
                            <p class="text-sm text-brand-text-muted mt-1">${t('artsmoker.style_library.subtitle')}</p>
                        </div>
                        <button class="btn-create-style btn btn-primary">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            ${t('artsmoker.style_library.create_new')}
                        </button>
                    </div>
                ...
                <div id="style-modal" class="hidden fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                    <div class="modal-content bg-brand-surface rounded-xl border border-brand-border shadow-2xl w-full max-w-lg">
                        <div class="flex items-center justify-between px-6 py-4 border-b border-brand-border">
                            <h3 id="style-modal-title" class="text-lg font-semibold">${t('artsmoker.style_library.modal_title_create')}</h3>
                        ...
                        <form id="style-form" class="p-6 space-y-4">
                            ...
                            <div>
                                <label class="block text-sm font-medium mb-1" for="style-name">${t('artsmoker.style_library.name')}</label>
                                <input id="style-name" class="input" type="text" placeholder="${t('artsmoker.style_library.name_placeholder')}" required>
                            </div>
                            <div>
                                <label class="block text-sm font-medium mb-1" for="style-description">${t('artsmoker.style_library.description')}</label>
                                <textarea id="style-description" class="input" rows="3" placeholder="${t('artsmoker.style_library.description_placeholder')}"></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium mb-1" for="style-hints">${t('artsmoker.style_library.hints')}</label>
                                <textarea id="style-hints" class="input" rows="2" placeholder="${t('artsmoker.style_library.hints_placeholder')}"></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium mb-1" for="style-import-path">${t('artsmoker.style_library.import_from')}</label>
                                <div class="flex gap-2">
                                    <input id="style-import-path" class="input flex-1" type="text" placeholder="${t('artsmoker.style_library.import_placeholder')}">
                                    <button type="button" id="btn-browse-local" class="btn btn-secondary btn-sm whitespace-nowrap">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"/>
                                        </svg>
                                        ${t('artsmoker.style_library.browse_local')}
                                    </button>
                                    <button type="button" id="btn-browse-s3" class="btn btn-secondary btn-sm whitespace-nowrap">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002 5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z"/>
                                        </svg>
                                        ${t('artsmoker.style_library.browse_s3')}
                                    </button>
                                </div>
                                <p class="text-[10px] text-brand-text-muted mt-1">${t('artsmoker.style_library.import_desc')}</p>
                            </div>
                            <div class="flex justify-end gap-2 pt-2">
                                <button type="button" class="btn-cancel-modal btn btn-secondary">${t('artsmoker.common.cancel')}</button>
                                <button type="submit" class="btn btn-primary">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                    </svg>
                                    ${t('artsmoker.common.save')}
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            if (this._styles.length === 0) {
                ...
                grid.innerHTML = html`
                    <div class="col-span-full empty-state py-16">
                        ...
                        <h3 class="text-lg font-semibold text-brand-text mb-1">${t('artsmoker.style_library.no_styles')}</h3>
                        <p class="text-brand-text-muted text-sm mb-4">${t('artsmoker.style_library.no_styles_desc')}</p>
                        <button class="btn btn-primary btn-sm btn-create-first">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            ${t('artsmoker.style_library.create_style_short')}
                        </button>
                    </div>
                `;
            ...
            return html`
                <div class="style-card card cursor-pointer overflow-hidden group" data-id="${style.id}">
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-sm text-brand-text-muted mt-1 line-clamp-2">${style.description || t('artsmoker.style_library.no_description')}</p>
                        <div class="flex items-center gap-3 mt-3 text-xs text-brand-text-muted">
                            ...
                            ${style.analyzed_style ? html`<span class="badge badge-green">${t('artsmoker.style_library.analyzed')}</span>` : ''}
                        </div>
                    </div>
                </div>
            `;
        ...
        async _showDetail(id) {
            ...
            try {
                ...
                // nosemgrep
                overlay.innerHTML = html`<div class="text-center py-12 text-red-400">${t('artsmoker.style_library.detail_failed')}</div>`;
            }
        },
        ...
        _detailHTML(style) {
            ...
            return html`
                <div class="card-static p-6 space-y-6 fade-in">
                    ...
                    <div>
                        <h3 class="text-sm font-semibold text-brand-text-muted uppercase tracking-wider mb-2">${t('artsmoker.style_library.generation_hints_label')}</h3>
                        <p class="text-sm p-3 rounded-lg bg-brand-bg/60">${style.generation_hints}</p>
                    </div>` : ''}

                    <!-- Reference Images -->
                    <div>
                        <h3 class="text-sm font-semibold text-brand-text-muted uppercase tracking-wider mb-3">${t('artsmoker.style_library.ref_images_count', { count: refs.length })}</h3>
                        ...
                        <div class="upload-zone p-8 text-center" id="ref-upload-zone" data-style-id="${style.id}">
                            ...
                            <p class="text-sm text-brand-text-muted mb-1">${t('artsmoker.style_library.upload_hint')}</p>
                            <p class="text-xs text-brand-text-muted/60">${t('artsmoker.style_library.upload_hint_sub')}</p>
                        ...
                        <div class="mt-4 p-4 rounded-lg border border-brand-border bg-brand-bg/40 space-y-3">
                            <h4 class="text-sm font-medium flex items-center gap-2">
                                ...
                                ${t('artsmoker.style_library.import_dir_s3')}
                            </h4>
                            <div class="flex gap-2">
                                <input
                                    type="text"
                                    ...
                                    class="input flex-1"
                                    placeholder="${t('artsmoker.style_library.import_placeholder')}"
                                />
                                ...
                                <button id="btn-import-path" class="btn btn-primary btn-sm whitespace-nowrap">
                                    ...
                                    ${t('artsmoker.style_library.import_analyze')}
                                </button>
                            </div>
                            <p class="text-[10px] text-brand-text-muted/60">${t('artsmoker.style_library.import_desc_detail')}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <button class="btn-analyze btn ${style.analyzed_style ? 'btn-secondary' : 'btn-primary'}" data-id="${style.id}">
                            ...
                            ${style.analyzed_style ? t('artsmoker.style_library.reanalyze') : t('artsmoker.style_library.analyze')}
                        </button>
                        <span class="text-xs text-brand-text-muted">${style.analyzed_style
                            ? t('artsmoker.style_library.reanalyze_desc')
                            : t('artsmoker.style_library.analyze_first_desc')
                        }</span>
                    </div>` : ''}
                    ...
                    ${analyzedJson ? html`
                    <div>
                        <h3 class="text-sm font-semibold text-brand-text-muted uppercase tracking-wider mb-2">${t('artsmoker.style_library.analyzed_style_label')}</h3>
        ...
        _attachDetailEvents(style) {
            ...
            document.querySelector('.btn-delete-style')?.addEventListener('click', async () => {
                if (!await window.showConfirm(t('artsmoker.style_library.delete_confirm', { name: style.name }), { title: t('artsmoker.style_library.delete_title'), detail: t('artsmoker.style_library.delete_detail'), confirmLabel: t('artsmoker.common.delete'), danger: true })) return;
                try {
                    window.showLoading && window.showLoading(t('artsmoker.style_library.deleting'));
                    await API.styles.delete(style.id);
                    window.hideLoading && window.hideLoading();
                    window.showToast && window.showToast(t('artsmoker.style_library.deleted'), 'success');
                    this._activeDetail = null;
                    const overlay = document.getElementById('style-detail-overlay');
                    if (overlay) overlay.classList.add('hidden');
                    await this._loadStyles();
            ...
            document.getElementById('btn-import-path')?.addEventListener('click', async () => {
                ...
                if (!path) {
                    window.showToast?.(t('artsmoker.style_library.enter_path'), 'warning');
                    return;
                }
                const btn = document.getElementById('btn-import-path');
                const origHTML = btn.innerHTML;
                // nosemgrep
                btn.innerHTML = html`<span class="spinner-sm"></span> ${t('artsmoker.style_library.importing_analyzing')}`;
                btn.disabled = true;
                try {
                    await API.styles.importPath(style.id, path, true);
                    window.showToast?.(t('artsmoker.style_library.imported'), 'success');
            ...
            document.querySelector('.btn-analyze')?.addEventListener('click', async () => {
                ...
                const origHTML = btn.innerHTML;
                // nosemgrep
                btn.innerHTML = html`<span class="spinner-sm"></span> ${t('artsmoker.style_library.analyzing')}`;
                btn.disabled = true;
                try {
                    await API.styles.analyze(style.id);
                    window.showToast && window.showToast(t('artsmoker.style_library.analysis_complete'), 'success');
                    // Refresh detail
                    this._showDetail(style.id);
                } catch (err) {
                    // nosemgrep
        ...
        async _uploadRefs(styleId, files) {
            window.showLoading && window.showLoading(t('artsmoker.style_library.uploading'));
            try {
                await API.styles.uploadReferences(styleId, files);
                window.hideLoading && window.hideLoading();
                window.showToast && window.showToast(t('artsmoker.style_library.uploaded', { count: files.length }), 'success');
        ...
        _openModal(style = null) {
            ...
            if (style) {
                title.textContent = t('artsmoker.style_library.modal_title_edit');
                ...
                title.textContent = t('artsmoker.style_library.modal_title_create');
                idField.value = '';
                nameField.value = '';
                descField.value = '';
                hintsField.value = '';
        ...
        async _handleSave() {
            ...
            if (!name) {
                window.showToast?.(t('artsmoker.style_library.name_required'), 'warning');
                return;
            }
            ...
            try {
                ...
                if (styleId) {
                    window.showLoading?.(t('artsmoker.style_library.saving'));
                    await API.styles.update(styleId, data);
                    window.showToast?.(t('artsmoker.style_library.updated'), 'success');
                } else {
                    window.showLoading?.(t('artsmoker.style_library.creating'));
                    const created = await API.styles.create(data);
                    styleId = created?.id;
                    window.showToast?.(t('artsmoker.style_library.created'), 'success');
                }

                // Import references: browsed files take priority over path
                if (this._browsedFiles && this._browsedFiles.length > 0 && styleId) {
                    window.showLoading?.(t('artsmoker.style_library.uploading'));
                    try {
                        ...
                        if (imageFiles.length > 0) {
                            ...
                            window.showLoading?.(t('artsmoker.style_library.analyzing'));
                            await API.styles.analyze(styleId);
                            window.showToast?.(t('artsmoker.style_library.images_uploaded_analyzed', { count: imageFiles.length }), 'success');
                        } else {
                            window.showToast?.(t('artsmoker.style_library.no_images_found'), 'warning');
                        }
                    } catch (err) {
                        window.showToast?.(t('artsmoker.style_library.upload_failed', { msg: err.message }), 'warning');
                    }
                    this._browsedFiles = null;
                } else if (importPath && !importPath.startsWith('[Browse]') && styleId) {
                    window.showLoading?.(t('artsmoker.style_library.importing_refs'));
                    try {
                        await API.styles.importPath(styleId, importPath, true);
                        window.showToast?.(t('artsmoker.style_library.imported'), 'success');
                    } catch (err) {
                        window.showToast?.(t('artsmoker.style_library.import_failed', { msg: err.message }), 'warning');
                    }
                }
        ...
        _openBrowseModal(mode, targetInputId) {
            ...
            modal.innerHTML = html`
                <div class="bg-brand-surface rounded-xl border border-brand-border shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-3 border-b border-brand-border">
                        <h3 class="text-base font-semibold">${mode === 's3' ? t('artsmoker.style_library.browse_s3_title') : t('artsmoker.style_library.browse_local_title')}</h3>
                    ...
                    <div class="px-5 py-3 border-b border-brand-border">
                        <div class="flex gap-2 items-center text-sm">
                            <button id="browse-back-btn" class="p-1.5 rounded-lg hover:bg-white/5 text-brand-text-muted hover:text-brand-text transition-colors disabled:opacity-30 disabled:cursor-not-allowed" title="${t('artsmoker.misc.go_to_parent')}" disabled>
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                </svg>
                            </button>
                            <span class="text-brand-text-muted font-mono text-xs flex-1 truncate" id="browse-location">${t('artsmoker.common.loading')}</span>
                        </div>
                    </div>
                    ...
                    <div class="px-5 py-2 border-t border-brand-border">
                        <p class="text-[10px] text-brand-text-muted/50">${t('artsmoker.style_library.browse_hint')}</p>
                    </div>
                    <div class="px-5 py-3 border-t border-brand-border flex items-center justify-between">
                        ...
                        <div class="flex gap-2">
                            <button class="browse-cancel btn btn-secondary btn-sm">${t('artsmoker.common.cancel')}</button>
                            <button class="browse-select btn btn-primary btn-sm" id="browse-select-btn">${t('artsmoker.style_library.browse_select')}</button>
                        </div>
                    </div>
                </div>
            `;
            ...
            const renderList = (items, location, info, parent) => {
                ...
                if (items.length === 0) {
                    // nosemgrep
                    list.innerHTML = html`<p class="text-center text-brand-text-muted text-sm py-8">${t('artsmoker.style_library.browse_empty')}</p>`;
                    return;
                }
                // nosemgrep
