...
(function () {
    ...
    async function navigate() {
        ...
        if (!routeDef || !routeDef.component) {
            // nosemgrep
            app.innerHTML = html`<p class="text-center py-12 text-brand-text-muted">${t('artsmoker.ui.onboarding.page_not_found')}</p>`;
            currentRoute = null;
            return;
        }
    ...
    window.showToast = function (message, type, duration) {
        ...
        toast.innerHTML = html`
            ...
            <button class="toast-close p-0.5 rounded hover:bg-white/5 text-brand-text-muted hover:text-brand-text transition-colors flex-shrink-0" title="${typeof t !== 'undefined' ? t('artsmoker.ui.common.dismiss') : 'Dismiss'}">
        ...
        _syncModal.innerHTML = html`
            <div class="bg-brand-surface rounded-xl border border-brand-border shadow-2xl max-w-lg w-full p-8 space-y-4">
                <div class="text-center">
                    ...
                    <h3 class="text-sm font-semibold text-brand-text">${t('artsmoker.ui.onboarding.setup_title')}</h3>
                    <p class="text-xs text-brand-text-muted mt-1">${t('artsmoker.ui.onboarding.setup_desc')}</p>
                </div>
    ...
    function _closeSyncModal(hadError, errorMsg) {
        ...
        if (hadError) {
            ...
            inner.innerHTML = html`
                ...
                <h3 class="text-sm font-semibold text-amber-400">${t('artsmoker.ui.onboarding.discovery_failed')}</h3>
                <p class="text-xs text-brand-text-muted">${t('artsmoker.ui.onboarding.discovery_failed_desc')} <strong>${t('artsmoker.ui.onboarding.sync_from_aws')}</strong> ${t('artsmoker.ui.onboarding.before_using')}</p>
                <p class="text-[10px] text-brand-text-muted/50 mt-1">${errorMsg || t('artsmoker.ui.onboarding.unknown_error')}</p>
                <div class="flex gap-2 justify-center mt-4">
                    <button class="sync-open-settings btn btn-sm text-xs px-5 py-2 rounded-lg bg-brand-accent hover:bg-brand-accent-hover text-white font-medium">${t('artsmoker.ui.onboarding.open_model_settings')}</button>
                    <button class="sync-dismiss btn btn-sm text-xs px-5 py-2 rounded-lg border border-brand-border hover:bg-white/5 text-brand-text-muted">${t('artsmoker.ui.common.dismiss')}</button>
                </div>`;
            ...
            inner.innerHTML = html`
                <div class="text-3xl mb-2">✓</div>
                <h3 class="text-sm font-semibold text-brand-text">${t('artsmoker.ui.onboarding.ready_title')}</h3>
                <p class="text-xs text-brand-text-muted">${t('artsmoker.ui.onboarding.ready_desc')}</p>
                <button class="btn btn-sm text-xs px-6 py-2 mt-3 rounded-lg bg-brand-accent hover:bg-brand-accent-hover text-white font-medium">${t('artsmoker.ui.common.ok')}</button>`;
        }
    ...
    fetch('/api/ping', {
        ...
        body: JSON.stringify({
            ...
            browser: navigator.userAgent?.split(/[()]/)[1] || '',  // nosemgrep -- array index on a split result, not JSON-derived object keys
        ...
        function waitForServerRestart() {
            ...
            const poll = setInterval(async () => {
                ...
                if (elapsed >= timeout) {
                    ...
                    if (restartBanner) {
                        // nosemgrep
                        restartBanner.innerHTML = html`<span class="mr-2">⚠</span> ${t('artsmoker.ui.onboarding.server_timeout')} <button onclick="location.reload()" class="underline ml-2 font-semibold">${t('artsmoker.ui.onboarding.refresh')}</button>`;
