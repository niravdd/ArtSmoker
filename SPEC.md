...
- **Entry is isolated** (SPEC design principle: *reuse the engine, fork the entry*): a self-contained toggle + "Design the set…" button in `ReferenceStudio`, wired to `ImageStudio._designReferenceCollection` (reference → vision art direction → the **same** input-agnostic Collection Designer builds the roster) then `_generateReferenceCollection`. The Designer, store, generate engine, viewer, and the Text path are untouched.
...
- **Model support (async-aware — nothing forced):** the user's model selection is honored as-is — **sync Bedrock AND custom self-hosted (async) models** both work in a collection. Because an async self-hosted Batch submits its Jobs and returns before the image lands, the collection generate loop **waits** for each Batch's Jobs to reach a terminal state (poll every few seconds, up to `collection_async_batch_timeout_s`, default 15 min) before judging it, emitting a `batch_pending` heartbeat meanwhile; on timeout it moves on and the async-complete hook (`_update_gallery_on_complete` → `refresh_if_member`) finalizes the set when the straggler lands. A sync Bedrock Batch is already terminal on return → no wait. No model is ever substituted.
