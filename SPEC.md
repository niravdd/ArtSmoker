...
2. **Lifespan handler** (async context manager):
   ...
   - **Generated image assets live in `data/images/`** (was `data/generated/` before the 2026-09-08 rename, aligning the name with `data/styles`, `data/video`). `LocalStore.__init__` runs a **one-time, transparent migration**: if a legacy `data/generated/` exists and `data/images/` does NOT, it is atomically renamed (`Path.rename` — no copy, no data loss). If BOTH exist (can't auto-decide), startup leaves them untouched and the Gallery surfaces a consent prompt (`GET /api/gallery/storage-migration-status` → confirm → `POST /api/gallery/migrate-storage`) that moves legacy asset dirs into `data/images/`, skipping any id already present (never overwrites). Asset ids and the `{batch_id}_o{n}_v{m}` folder scheme are unchanged — only the parent dir moved.
