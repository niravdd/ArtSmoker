...
**Terminology (locked):** a **batch** = ONE press of Generate — the collection of everything it produces (options × variations × models), identified by `batch_id` and the unit of Gallery reload / regeneration / the base seed. A **job** = ONE generation inside a batch (a single image: one option × variation slot on one model); per-image asset ids are `{batch_id}_o{n}_v{m}`, and self-hosted jobs additionally carry an async job id. Never call a batch a "job" (or vice versa) in user-facing text — video/3D submissions produce one output each, so their "Job" labels are single-job batches and read correctly.
...
- **Reference-guided**: generate from 1–3 uploaded reference images plus a mandatory user instruction. Two sub-modes:
  - **"Match the reference"** — preserves the subject/product/character from the reference while applying the instruction (theme, background, wardrobe, lighting). Powered by a deployed reference-capable instruction editor — resolved from the registry (`capabilities.reference_guided` in the custom-model catalog), never hardcoded to a specific model. When one is deployed, the **main sidebar model dropdown is filtered** to the deployed instances (injected as temporary single-select entries — newest auto-selected — carrying their warm/cold economics so the cost estimate stays truthful; the panel shows the warm-vs-cold cost hint for the pick); when none is, the mode is **deploy-gated** with a message naming the supported catalog models (from `supported` in `GET /api/generate/reference-available`) and a route to Custom Models, mirroring the 3D-pipeline gating pattern. Genuinely single-model/single-concept: the backend (`_prepare_reference_generation`) forces `num_options=1` and clears any model fan-out.
  - **"Remix the reference"** — classic strength-based image-to-image on Bedrock: the FIRST reference image is forwarded (family `reference_image_paths` → `image`) with `extra_params {mode: "image-to-image", strength}` to a model carrying the registry capability flag `capabilities.image_to_image` (SD3.5 Large, Stable Image Ultra; `aspect_ratio` is omitted — output follows the input image). The **main sidebar model dropdown is filtered** to these capable models (single-select) — no separate chooser in the panel; the backend honors the pick when capable, else falls back to the first capable enabled model. No vision analysis, no deploy. `GenerationRequest.reference_strengths` carries one strength per option (the **strength ladder** — the sidebar Options control is relabeled "Strengths"; the same prompt runs at each strength, computed in the UI and used verbatim, clamped server-side to 0.05–0.95; DISPLAYED as whole percentages in 5% steps, while the slider/drafts/metadata/API all keep 0–1 decimals). The instruction gets ONE standard refine pass (style/model-aware, falls back to raw on refusal); each asset's metadata records its `reference_strength` for reload. Keeps composition/palette, NOT identity.
  - **"Inspired by the reference"** — vision-analyzes the reference(s) + instruction into enhanced prompt(s) (`POST /api/generate/analyze-reference`), then generates with the user's normal text-to-image models. **Always available** — needs no custom deployment. **Fully honors the sidebar selection**: multiple selected models fan out through the all-models pipeline, and Options > 1 asks the vision LLM for that many DISTINCT interpretations of the same reference + instruction (one per option; variations vary the seed). The derived prompts are previewed as **editable** textareas — one per option — and edited prompts are sent as `reference_enhanced_prompts`, used verbatim by the backend (no second vision call, no drift from what was shown). Per-model prompt optimization is forced off for reference jobs — the vision analysis IS the single enhancement pass. On Gallery reload, the enhanced prompt that drove the batch is shown read-only in the panel.
...
- **Edit tab** — five editing modes:
  ...
  - Models populated dynamically from registry filtered by `model_purpose` (inpainting, erase, outpainting, search_replace, or search_recolor). Shows price per model. **Instruction editors** (`model_purpose: image_edit`, e.g. Qwen-Image-Edit) appear across all five modes via their `capabilities` map and need no mask.
  - **Mask gate**: for mask-requiring model+mode combos, **Apply Edit is disabled until a mask is painted** — the persistent status line explains why ("Paint a mask on the image to enable Apply Edit"). Clearing the mask or switching to a mask-requiring model re-arms the gate; switching to a mask-free editor lifts it. Prevents silent no-op submits.
...
**Self-healing on auth failure** (`_mantle_call`): every Mantle invoker runs through a wrapper that, on a `401`/`403`/expired-token error, discards the cached/env token, **force-derives** a fresh short-term token from the AWS credentials, and retries the call **once**. This recovers automatically from a stale `AWS_BEARER_TOKEN_BEDROCK` (e.g. an operator-set short-term token that lapsed) without ever mutating `os.environ` — the operator's value stays as they set it, and the retry simply uses an in-memory derived token. Applies to all three Mantle APIs (Chat Completions, Responses, Messages).
...
6. Initialize internal services, start server
...
| GET | `/api/generate/reference-available` | Whether a reference-capable editor is deployed. Gates the "Match the reference" mode in the Image Studio. Returns `{available, model_key, endpoint_name, deploy_catalog_key, models, supported}` — `models` lists ALL deployed reference-capable instances (newest first, each with its `economics`; powers the match-mode filtering of the main model dropdown + the panel's warm/cold cost hint) and `supported` lists the catalog models that CAN do this once deployed (registry-driven via `capabilities.reference_guided`; powers a generic "deploy one of these" message — never hardcoded to a specific model). "Inspired by the reference" mode needs no custom model and is always available. |
| POST | `/api/generate/analyze-reference` | Vision-analyze 1–3 reference images + a mandatory user instruction → enhanced prompt(s), for the "Inspired by the reference" preview. Body (`AnalyzeReferenceRequest`): `images` (1–3 base64 PNGs), `prompt` (required — what to do with the reference), `asset_type`, `ui_lang` (soft translation hint), `num_options` (1–5 — one DISTINCT interpretation per generation option). Returns `enhanced_prompts` (array, one per option; shown as editable textareas), `enhanced_prompt` (first, for compat), plus subject/preserve/intent/negative/notes and accumulated cost. |
...
| DELETE | `/api/gallery/{id}/version/{version}` | Delete ONE version: its files (PNG/SVG + all version-named sidecars: mask, cutout, source, export artefacts, 3D GLBs + the `three_d` bucket) and metadata references. The `versions[]` record becomes a **tombstone** `{version, deleted: true, deleted_at}` — numbering is **sparse** (later versions keep their numbers; a future edit uses max+1, never reusing a deleted number). Deleting the CURRENT version **promotes** the next-lower survivor (its bytes become `asset.png`/`asset.svg`, `current_version` repoints). Deleting the LAST survivor deletes the whole asset (`{asset_deleted: true}`). Corruption-safe ordering: validate → promote (copy) → save metadata → remove files last; per-file failures are collected in `file_errors` (orphans, not corruption). 409 on an already-deleted version. Serialized per-asset via the same write lock as the edit writers. |
| GET | `/api/gallery/{id}/version-svg/{version}` | Download the SVG for a specific version (falls back to `asset.svg` for the current version). |
| GET | `/api/gallery/{id}/export-status?version=N` | Export & Cutouts: which artefacts (cutout PNG / no-bg SVG / with-bg SVG) exist for the version, plus the method used. |
| POST | `/api/gallery/{id}/export-variants` | Create the export artefacts for a version. Body: `{method: "local"\|"bedrock", version}`. Idempotent per method (re-running with the same method reuses); switching method regenerates. Local = free (rembg); bedrock cost is recorded in `meta["cutouts"]` + cost history. |
| GET | `/api/gallery/{id}/cutout-png/{version}` | Download the background-removed transparent PNG cutout for a version. |
| GET | `/api/gallery/{id}/cutout-svg/{version}` | Download the no-background SVG (vector trace of the cutout) for a version. |
...
**Blender provisioning (zero end-user setup).** Reuse-first detection cascade: managed `tools/` copy → PATH → OS-standard paths, then version-gate (≥3.0) and a smoke-test that actually enables the glTF importer + FBX exporter ("exists" ≠ "works"). If none passes, the current-stable portable build (resolved via endoflife.date) downloads silently into repo-root `tools/` (gitignored). Runs once per process, then cached. A safe self-updater applies **only to the managed copy** (never a user's install): download-alongside → smoke-test the new build → switch only if it passes; on-demand via Model Settings → Maintenance ("Update Blender", shows version + path), plus an opportunistic check capped to ~30 days. On Linux, Blender needs ~11 X11/GL client libs even headless (OS-managed prereq; runtime-checked with a one-line `apt install` hint).

**Endpoint** — `GET /api/gallery/{asset_id}/3d/{version}/export/{fmt}?variant=&target=&pack=&lods=&collision=&uv2=&check=&prepare=`; `fmt ∈ glb|fbx|usd`.

- **Targets** (config-driven `mesh_export.TARGETS`): generic, unreal (Z-up), unity, godot, maya, 3dsmax — mapped to FBX `axis_up/axis_forward` and USD up/forward selections. USD exports as **.usdz** (packaged, textures inside). GLB is never re-oriented (glTF is spec-locked Y-up).
- **Prep ops** (user-chosen, validated per target via `normalize_export_ops` — unknown → `none`): `pack` (orm | unity_mask | hdrp_mask; per-engine lists — Unity cannot consume UE-style ORM), `lods` (lod3 → LOD0–3 decimation chain 100/50/20/5% under a `LodGroup` FBX node; `_LOD{n}` names = Unity convention), `collision` (convex hull via trimesh | CoACD decomposition; hulls ride inside the export named `UCX_*`, or `-convcolonly` for Godot), `uv2` (second smart-projected UV layer; render UVs stay active). Texture packing runs in the app venv (trimesh + Pillow channel ops; missing AO fills white) and turns the artifact into a **ZIP** (model + `textures/`); it is force-`none` for `fmt=glb` (glTF's material model already embeds metallic-roughness). Packing on an untextured or albedo-only asset **fails fast with a 400** before Blender runs (`assert_packable`).
- **Raw vs processed GLB**: `fmt=glb` with no ops returns the pristine original (never regenerated). With ops, a **separately-named processed GLB** is built; the original is never touched.
- **Two-step UX**: `check=1` = cheap cached-status probe (drives the Generate ⇄ Download ✓ button state + the "Ready to download" chips); `prepare=1` = convert + cache + record without streaming the file (the "Generate" click); a plain GET downloads (converting lazily if needed). One Blender pass per request; only the requested format is built.
- **Caching & concurrency**: each `(version, variant, target, ops)` combination caches as its own file beside the GLB (`asset_3d_v{N}[__{variant}]__{target}[__{ops-token}].{ext}[.zip]`) — combinations never overwrite; a cross-worker `named_write_lock` per combination means duplicate/concurrent requests wait and reuse the first run's cache. Conversion runs off the event loop (`asyncio.to_thread`).
- **Provenance**: every successful export is recorded once in the asset's metadata under `three_d_exports` (format, target, ops, variant, version, zip, size, duration, Blender version, timestamp) under the per-asset write lock; the AssetViewer's chip row renders from it. Failures emit `telemetry.track_error` + an operator `logger.error` (asset/fmt/target/ops) and surface as a clean 503/400 toast.
- **Fidelity note** (surfaced in UI): FBX has no native metallic-roughness slot — geometry/UVs/base color/normal survive; metallic-roughness re-hooks on engine import. The packed texture files travel alongside so nothing is lost.
...
## 9. Application Bootstrap (app.py)

`backend/app.py` assembles the FastAPI application (`backend/main.py` is the thin supervisor/entry shim that lazily re-exports `app`). Build it in this exact order:
...
>
> **Custom-hosted (SageMaker) display vs actual cost:** the UI shows the instance's **hourly running rate + typical generation time** in Model Settings / the catalog (a per-image price is misleading — idle + spin-up bill on top of the generation); the Image Studio estimate uses the derived per-generation compute cost (`hourly × typical_latency/3600`). The **actual incurred cost of a job** = `hourly × (submit→completion runtime, incl. cold-start) + amortized cooldown share` (cooldown is latency-derived, `max(600, typical×2)`), computed at the same rate shown. Standalone auxiliary LLM operations (prompt classify/decompose/recompose, analyze-reference, pre-screen, analyze-moderation, suggest-edit-prompt, template enhance/auto-fix, chat compact/title, typestudio suggest) each scope their own cost via `reset_costs()` / `get_total_cost()`.
...
- **Creativity everywhere.** The roster step recognizes a canonical slot-structure (chess = 6 piece types; deck = 52; tarot = 78) *or* invents one, but always reimagines each slot richly in-theme. How creative-vs-faithful it leans is a **registry-driven** prompt (`prompt_templates.json`), tunable with no code.
...
- **Asset-Type check BEFORE any spend.** A set is Character or Game Asset only (§18.12), and a whole set in the wrong framing is expensive to redo — so `ImageStudio._confirmCollectionAssetType(prompt)` runs **before the first paid call** of every collection entry: checking the toggle (before the art-direction-fields call; checkbox stays disabled during the check), opening the Collection Designer, and designing an Image-Inspired set (before the vision call). It POSTs `/api/refine-prompt/classify-asset-type` with `collection: true`, which uses the two-way `collection_asset_type_classify` template (chess/board-game pieces → `game_asset`) and returns `unsupported: true` when the sidebar type can't apply to a set. **Unsupported** (e.g. Photorealistic left selected) → a dialog that must resolve to *Use {suggested}* / *Use {the other}* (Escape/backdrop → suggested, via `showConfirm({dismissResult})`); **supported mismatch** (e.g. Game Asset for a party of heroes) → *Use {suggested}* / *Keep {current}*. The choice is written to the sidebar select; the check is cached per prompt (re-checks are free; a reloaded collection is pre-marked). If the classifier is unreachable an unsupported type is coerced to Game Asset with a toast (the backend's own coercion, made visible). While a collection is active in the current tab, the non-set `<option>`s are **disabled** (`_syncAssetTypeLock`, driven by `_syncGenerateGate`).

**Two entries — Text-Inspired and Image-Inspired.** Everything above is the **Text-Inspired** path (the ask drives the art direction), living in the **Text** tab (`PromptEditor`). The **Image-Inspired** path lives in the **Reference** tab (`ReferenceStudio`) and is **the same flow with a different input**: the reference image's LOOK inspires the art direction.
- It is **"Inspired by the reference" only** — Match and Remix are single-image transforms (preserve/img2img one subject) that don't map onto a set of *distinct* subjects, so the collection toggle only appears in **inspired** mode.
- The reference image(s) + instruction feed the art-direction step via a **vision** call: `generate_art_direction` / `recommend_art_direction_fields` take optional `reference_images` (bytes) and, when present, route to a vision model that derives the aesthetic from what the images show (an artist's hand-drawn base, a mood board). The **roster stays text-driven** — the image shapes the *style*; the instruction defines *what the set is*. The endpoints `/art-direction`, `/art-direction-fields`, `/decompose` accept `reference_images` (b64/data-URL, ≤3, decoded via `_decode_ref_images`; a bad image is skipped, never fatal — falls back to text).
- **Entry is isolated** (SPEC design principle: *reuse the engine, fork the entry*): a self-contained toggle + **🗂️ Collection Designer** button in `ReferenceStudio`, wired to `ImageStudio._designReferenceCollection` (reference → vision art direction → the **same** input-agnostic Collection Designer builds the roster) then `_generateReferenceCollection`. The Designer, store, generate engine, viewer, and the Text path are untouched.
- **UX parity with the Text flow** (identical gating, so no work fires with no input): the Collection toggle is **disabled until ≥1 reference image is supplied** (the Text flow gates on a typed prompt); the Collection Designer is **mandatory** and **Generate stays disabled until it's accepted** (shared `_syncGenerateGate()`); the single-asset "Enhanced Prompt" preview is **hidden in collection mode** (a collection's shared art direction — reviewed/edited inside the Designer — replaces it); and an accepted design shows a read-only decided-summary (subjects · O×V · images · models). The checkbox/label formatting matches the Text toggle.
- **Visual anchor (cohesion_mode `reference`).** For a single-model set, every Batch is anchored to the supplied image via the existing single-asset "inspired" path (vision fuses the reference look with each subject's prompt) so the set adheres to the aesthetic. The image(s) are **persisted** to the collection dir (`ref_N.png`, `record.reference_images`, `knobs.image_inspired`) so a per-Batch **retry re-anchors identically**; `delete` rmtrees the dir (no leak). A **multi-model** image-inspired set falls back to art-direction-only fan-out (the anchor is single-model, mirroring hero-anchor — the reference still shaped the shared art-direction text).
- **Model support:** a collection generates on any selected image model. A sync Bedrock Batch's Jobs are on disk when generation returns; an async self-hosted Batch submits its Jobs and lands them later via the background poller, so the generate loop waits for each Batch's Jobs to reach a terminal state — polling up to `collection_async_batch_timeout_s` (default 15 min) and emitting a `batch_pending` heartbeat — before judging it. On timeout the loop proceeds and the async-completion hook (`_update_gallery_on_complete` → `refresh_if_member`) finalizes the set when the image lands.
- **Reproducibility caveat:** a `reference`- (or `hero`-) cohesion set is **not** byte-reproducible — the per-Batch "inspired" anchor derives each prompt via a nondeterministic vision call, so `saved_concept_prompts` byte-identity applies only to the default `prompt` cohesion. (Consistent with the single-asset "inspired" + hero-anchor paths.)
...
- **Knobs (with a live cost estimate):** Batch count N — shown as the **actual roster count** with an *auto-defined / user-defined* label (changing it re-fans the roster to that count, locked rows kept); options O, variations V, model(s) (the sidebar model selection is honored — **one** model → a cohesive single-model set; **multiple** → every subject renders on *each* chosen model, a cross-model comparison set, and the count multiplies the projected images/cost), cohesion tier (§18.5).
...
**Model selection (single vs multi).** The sidebar model checkboxes drive the render. **One** model → a cohesive single-model set (each subject's Batch runs the model-agnostic prompt verbatim per option). **Multiple** models → the user opted into a cross-model comparison, so **every subject renders on every chosen model**: each subject stays ONE Batch, generated through the shared all-models engine (`_run_all_models_generation`) — it reuses the saved model-agnostic prompt verbatim per model (no concept re-fan), applies the set-wide negative + background-removal, and produces flat options whose per-option `model_label` the Collection Asset Viewer tags. Total images = Batches × Models × Options × Variations. Hero-anchor cohesion is single-model only (it does not compose with a multi-model set).
...
- The **Collection Asset Viewer** is the collection-level counterpart to the batch view + AssetViewer:
  ...
  - **Selection → Convert to 3D** — checkboxes select whole batches or individual jobs; **Convert to 3D** opens a **single settings pane** whose choices apply **uniformly** to every selected job (or, when nothing is selected, each Batch's representative image — o0_v0 at its pinned/current version), running the existing per-asset image-to-3D pipeline once per target. The user picks *what* to mesh (partial/blank images are theirs to judge). The pane has **parity with the AssetViewer 3D tab** via shared `AssetViewer.threeD*` helpers + `THREE_D_QUALITY_PRESETS` (single source): pipeline chooser (`model_key`, warming state), license panel, quality Fast/Standard/High (default High) mapped to steps/guidance/max-faces/octree depth, Advanced (steps, guidance, faces, depth, seed), save-as (new version / new variant), per-job estimate **× N total** (time + Est. cost), a **confirm of the total before any spend**, the S3-bucket preflight, and a not-deployed state linking to Model Settings. Settings sent: `{model_key, quality, steps, guidance, max_faces, mesh_resolution, save_as, seed?}`.
  - **Header actions** — **Load in Image Studio** is the first, primary, focused (Enter) action, followed by Convert to 3D, downloads, Delete, close. Header handlers bind once at mount (not per re-render).
  ...
  - **Load in Image Studio** — reload a whole collection back into the studio for review/regeneration (mirrors the single-asset AssetViewer reload). `ImageStudio.loadCollection(id)` fetches the master record, restores the sidebar (models, asset type, O·V, seed, remove-bg — record `knobs {O,V,cohesion_mode,remove_background}` mapped to the Designer shape `{options,variations,cohesion,removeBg}`) and routes to the right tab: a **text** collection restores into the PromptEditor (`restoreCollectionDesign` — re-enters collection mode with the persisted art direction + roster as an already-accepted design); an **image-inspired** one restores into the Reference tab (`ReferenceStudio.loadCollection` repopulates the instruction + reference images, inspired + collection on; `_refCollectionDesign` restored). Generate is enabled (a re-run); the Designer stays available to tweak. Image-inspired reference bytes are served by **`GET /api/collections/{id}/reference/{ref_N.png}`** (regex-restricted, no traversal) from the persisted `record.reference_images` (`ref_N.png` in the collection dir).
- **Image Studio "Your Collections" panel.** Below the preview, Image Studio lists the same collection cards as the Gallery (shared `Gallery.collectionCardHTML`; newest first, latest 4 + *Show all (N)*; hidden when there are none). A click opens the **same Collection Asset Viewer in place** — no trip to the Gallery. Refreshed on Studio init/show, after either collection generation flow, and on collection delete. The post-generation summary's button is **Open Collection** (opens the viewer in place).
- **Single-job drill-down carries collection lineage (AssetViewer).** Each Job's metadata is stamped with `collection_id`, `collection_name`, `batch_name`, `batch_slug`, `model_agnostic_prompt` (`_stamp_collection_lineage`, post-hoc under `asset_write_lock`). The single-asset **Asset Viewer** surfaces this: a **Collection** facts section (name + subject + an **Open collection** button → the Collection Asset Viewer) plus the **Collection prompt** (the subject's model-agnostic prompt, from the shared art direction) in the Prompt Lineage.
...
- **New:** the Collection toggle (Text + Image-Inspired entries), the Collection Designer + the per-Batch **Art Direction Controller** (opt-in lift to the shared art direction), the **dynamic art-direction field recommender** + guided Step-2 scaffold, **vision-driven art direction** from reference images, the roster fan-out + per-Batch model-agnostic prompt templates (`prompt_templates.json`), the `data/collections/**` store + Collection record (+ persisted `ref_N.png` served by `GET /{id}/reference/{ref_N.png}`), **multi-model fan-out** + the **async-aware per-Batch wait**, the Collection Asset Viewer (coverflow card, Details panel, selection, **selective Convert-to-3D settings pane**, **collection/3D downloads**, per-job 3D tags, **Load in Image Studio** reload), the collection-membership metadata fields (incl. `collection_name`) + the **AssetViewer Collection lineage panel**, the shared **Generate-gate** (`_syncGenerateGate`), and the LLM cost ledger.
...
- **Asset-type scope.** A collection's `asset_type` is coerced to **Character** or **Game Asset** (both enforce single-complete-subject framing + are 3D-ready). `game_asset` is a broad catch-all (chess pieces, cards, icons, props, emblems…). Dedicated `icon`/`environment`/`photorealistic`/`marketing_banner` framings are intentionally out of scope for collections. The UI resolves the type with the user **before** any spend (§18.2 Asset-Type check) rather than silently coercing.
- **Multi-model collections** are honored: the sidebar model selection drives the render — ONE model → a cohesive single-model set; MULTIPLE → every subject renders on **each** chosen model (a cross-model comparison), reusing the all-models engine per subject so each subject stays one Batch whose flat options carry per-model labels. Total images = Batches × Models × O × V (the estimate/summary/Gallery card multiply accordingly). Hero- and reference-anchor cohesion are single-model only (they don't compose with a multi-model set → such a set falls back to art-direction-only).
- **Async-aware generation.** A collection generates on any selected model. Sync Bedrock Batches are terminal when generation returns; an async self-hosted Batch submits its Jobs and lands them later via the poller, so the generate loop **waits** for each Batch's Jobs to reach a terminal state (up to `collection_async_batch_timeout_s`, default 15 min, emitting a `batch_pending` heartbeat) before judging it. On timeout it proceeds and the async-completion hook (`_update_gallery_on_complete → refresh_if_member`) finalizes the set when the image lands.
- **Reproducibility caveat.** A `reference`- or `hero`-cohesion set is **not** byte-reproducible — the per-Batch "inspired" anchor derives each prompt via a nondeterministic vision call; `saved_concept_prompts` byte-identity applies only to the default `prompt` cohesion.
