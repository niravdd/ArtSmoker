...
- **Model gating:** reference collections are limited to **sync Bedrock text-to-image** models (custom self-hosted async models are excluded — the collection generate loop assumes sync landing; if the user had only async models selected, a sync model is substituted with a notice).
