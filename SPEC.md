...
- **Left sidebar** (progressive disclosure layout):
  ...
  - **Seed** (visible number field below Options × Variations, not hidden in Advanced): pre-populated with a CLIENT-generated random value cached in `localStorage` (`artsmoker_gen_seed` — page reloads keep it), with a 🎲 re-roll button. Sent as `GenerationRequest.seed` (0 … 2³¹−1, pydantic-validated; blank = server-random, the legacy behavior). The backend NEVER overrides a provided seed: each (option, variation) slot derives a deterministic distinct seed via `_derive_seed(base, oi, vi, n_vars)` = `(base + oi·n_vars + vi) mod 2³¹` — so o0_v0 ≡ base, the same base + settings reproduce the same batch byte-for-byte (E2E-verified on SD3.5), and in an all-models run the same (concept, variation) slot shares its seed across models for comparable outputs. Every asset's metadata records its own final seed (shown in the AssetViewer Metadata tab); `GET /api/gallery/batch/{id}` returns the batch base seed (root) + per-variant seeds, and Image Studio restores the base on load — **clicking any option/variant result anchors the seed field to that image's seed** ("branch from this one"). Loading a batch no longer disables the Upscale toggle (the old lock blocked regenerating with upscale and was never released).
