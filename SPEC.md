...
- **Left sidebar** (progressive disclosure layout):
  ...
  - **Seed** (visible number field below Options × Variations, not hidden in Advanced): pre-populated with a CLIENT-generated random value cached in `localStorage` (`artsmoker_gen_seed` — page reloads keep it), with a 🎲 re-roll button. Sent as `GenerationRequest.seed` (0 … 2³¹−1, pydantic-validated; blank = server-random, the legacy behavior). The backend NEVER overrides a provided seed: each (option, variation) slot derives a deterministic distinct seed via `_derive_seed(base, oi, vi, n_vars)` = `(base + oi·n_vars + vi) mod 2³¹` — so o0_v0 ≡ base, the same base + settings reproduce the same batch byte-for-byte (E2E-verified on SD3.5), and in an all-models run the same (concept, variation) slot shares its seed across models for comparable outputs. Every asset's metadata records its own final seed (shown in the AssetViewer Metadata tab); `GET /api/gallery/batch/{id}` returns the batch base seed (root) + per-variant seeds, and Image Studio restores the base on load — **clicking any option/variant result anchors the seed field to that image's seed** ("branch from this one"). Loading a batch no longer disables the Upscale toggle (the old lock blocked regenerating with upscale and was never released). An **after-generation mode select** next to the field (persisted, `artsmoker_gen_seed_mode`) controls what the seed does when a run completes: **Auto-step** (default — advances by that run's options × variations, i.e. one past the highest derived slot, so consecutive runs never reuse a slot yet each stays reproducible from its own recorded base), **Keep** (exact repeat), or **Re-roll** (new random). The step is computed from the SUBMITTED seed, not the field (result-clicks may have re-anchored it).
