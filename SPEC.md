...
>
> The tables below are **reference pricing for deployment planning**. The application shows **live pricing** in the UI — Bedrock per-image/video/token prices are fetched from the AWS Pricing API (`_fetch_image_pricing`) and SageMaker instance hourly rates per-region (`_fetch_sagemaker_pricing`, `ServiceCode=AmazonSageMaker`, `component=Hosting`) during registry refresh-all, stored in `model_registry.json` (`image_pricing`, `sagemaker_pricing`). The single resolver `custom_models.get_instance_hourly_rate(instance, catalog_key, region)` prefers synced per-region rates, then any synced region, then catalog seeds — driving all compute-cost math (3D, async 2D, keep-warm). Model prices' `base_price_usd` / per-region `quality_prices` are displayed in the Image/Video Studio selectors and estimates. Pricing is cached in the registry and only updated when an admin runs refresh-all.
