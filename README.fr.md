...
- **2D, édité et en 3D — de bout en bout** — générez, puis affinez sur place avec l'inpainting, l'outpainting, la recoloration, le rechercher-remplacer et les éditions guidées par référence ; transformez n'importe quel asset 2D en un **modèle 3D entièrement texturé et adapté au moteur de jeu** qui s'intègre directement dans Unity, Unreal ou Blender — sans modélisation manuelle, dépliage UV ni peinture de textures. Plus de la vidéo cinématique et un studio de chat multi-modèle pour l'idéation.
...
- 🗂️ **Collections (génération d'ensembles)** — transformez UN prompt **ou une image de référence** en un **ensemble cohérent d'assets distincts** (« un jeu d'échecs de style viking », « 20 personnages JRPG »). Activez le sélecteur **Collection** dans l'Image Studio — depuis l'onglet **Text** (piloté par votre prompt) ou **Image-Inspired** (le *look* de l'image de référence pilote la direction artistique) — et un **Collection Designer** dédié décompose votre brief en une **direction artistique** partagée plus un **roster** de pièces distinctes dans le thème, chacune avec son propre prompt éditable et indépendant du modèle. Modifiez la direction artistique pour recomposer tout l'ensemble ; verrouillez, régénérez, ajoutez ou supprimez des pièces individuelles ; puis générez-les toutes d'un coup (options × variations chacune), avec une estimation en direct du design et du coût projeté. **Choisissez un seul modèle pour un ensemble homogène, ou plusieurs pour rendre chaque pièce sur chacun** (comparaison inter-modèles). Une cohésion optionnelle **hero-** / **reference-anchor** verrouille le style de l'ensemble sur une image choisie/fournie. Les collections apparaissent comme **une seule carte** dans la Gallery ; le visualiseur de collection permet à chaque pièce d'ouvrir l'Asset Viewer habituel (options / variations / versions, plus la **lignée de collection** de la pièce) ; vous pouvez **épingler la version** représentant chaque pièce, **passer tout l'ensemble en 3D** d'un coup, l'**exporter** en bundle par moteur, ou **recharger toute la collection dans l'Image Studio** pour la revoir et la régénérer. Vos collections apparaissent aussi **directement dans l'Image Studio** (mêmes cartes, visionneuse ouverte sur place), et **avant toute dépense** le type d'asset est vérifié par rapport à votre prompt (un ensemble = Personnages ou Assets de jeu) — pour ne jamais payer un ensemble entier dans le mauvais cadrage
...
- 🧊 **Génération Image-to-3D** — Convertissez n'importe quelle image Game Asset ou Character en un maillage 3D texturé (GLB) en un clic. La synthèse multi-vues + le baking de textures produit des assets entièrement texturés, adaptés au moteur de jeu. Visionneuse 3D interactive avec orbite/zoom/panoramique
...
*Les captures d'écran du pipeline 3D — génération, vérification de la source, export adapté au moteur de jeu et variantes — sont présentées plus bas, dans la section 1.9 (Génération de modèle 3D), aux côtés des fonctionnalités qu'elles illustrent.*
...
![Visionneuse 3D avec outils par variante et options d'export FBX/USD adaptées au moteur de jeu](docs/images/3d-model-viewer-export.png)
