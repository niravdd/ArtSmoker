...
- ✏️ **Édition d'images** — inpainting, outpainting, effacement, rechercher-remplacer, recoloration (dans l'AssetViewer). Chaque mode dispose d'un bouton IA **Générer le prompt** : un modèle de vision lit l'image et son prompt d'origine et propose un prompt d'édition adapté au mode et au modèle d'édition sélectionné (légende descriptive pour les éditeurs Stability, instruction pour Qwen-Image-Edit). Étendre/Outpaint affiche un aperçu en direct de l'agrandissement du canevas avec règles en pixels avant validation. Les éditeurs à instructions (Qwen-Image-Edit) prennent en charge **les cinq modes sans masque** — y compris une véritable extension du canevas : ArtSmoker pré-remplit le canevas, fait compléter uniquement la nouvelle zone par le modèle, puis réintègre vos pixels d'origine intacts. Chaque version éditée affiche **les deux étiquettes de modèle** — le générateur d'origine et l'éditeur de cette version
- 📤 **Export et détourages** — artefacts d'export par version dans l'AssetViewer : un détourage PNG transparent (fond supprimé) et de vraies vectorisations SVG (avec et sans fond). La suppression d'arrière-plan se choisit à chaque exécution : **gratuite sur l'appareil** (rembg/u2net, aucun coût cloud) ou via le suppresseur **payant Amazon Bedrock** — le même choix est proposé lors de la préparation des images pour la génération 3D
